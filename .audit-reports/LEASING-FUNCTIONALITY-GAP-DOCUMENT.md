...
|----|----------|-------|-----|------------------|
| G1 | Critical | Quotation | Creating a quotation throws an unhandled server error (undefined variable reference) | The sales funnel's entry point is dead. No new quotation can be created, which blocks every downstream stage. |
...
| G3 | Critical | Approval | The only approval screen renders hardcoded mock data and never calls the real approve endpoint | Every reviewer sees identical fake approval steps regardless of which quotation they open. Clicking "approve" changes nothing in the database. The approval gate does not gate anything. |
...
| G5 | High | Cross-cutting (nav) | 23 of 35 leasing page directories have no navigation entry, including the entire lessee master record and the entire billing chain (invoices, receipts, receivables, pre-billing, payments) | Staff cannot discover these screens without knowing the exact URL. 4 of the 23 are intentional retired-URL redirect stubs (`contracts`, `remarketing`, `direct-debits`, `branches`); the other 19 are real, working screens nobody can find. |
...
1. **G1** — quotations are the funnel entry point; nothing else matters if this is broken.
...
3. **G3** — approval UI wiring; the backend already exists, this is a frontend fix.
4. **G5** — nav exposure; cheap, and unlocks 19 already-built, already-working pages including the lessee master record.
