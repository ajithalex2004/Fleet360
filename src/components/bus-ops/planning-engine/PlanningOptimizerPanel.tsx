...
/**
 * Planning Optimizer — decision-support ranking dashboard, and the last
 * tab of the Planning Engine.
 *
 ...
 * mutated — this is a lens over the optimizer, not a scheduler. To act on
 * a ranking, the operator opens the winning plan on the Planning Core tab
 * and applies it there (which re-runs PCE via the same evaluator, so the
 * ranking's promises hold).
 *
 * Deliberate, and NOT relaxed by the move onto this page: no "apply from
 * here" button, and no hand-off that pre-selects the winning plan on
 * Planning Core. That separation prevents the "optimizer's #1 pick got
 * applied without another review" failure mode.
 *
 * It used to be enforced by distance — this was its own page, so acting on
 * a ranking meant navigating away and finding the plan yourself. As a tab
 * that distance is gone, so the rule now rests on the absence of an
 * affordance rather than on friction. Anyone adding a convenience link
 * from a ranked row into apply would be removing the control, not
 * improving the ergonomics. The hard control still holds either way: apply
 * re-runs PCE and returns 409 on a BLOCK verdict.
 */

import React, { useEffect, useState } from 'react';
import { Play, RefreshCw, Trophy, CheckCircle2, AlertTriangle, XCircle, Info } from 'lucide-react';
...
export function PlanningOptimizerPanel() {
  ...
  return (
    <div className="space-y-8">
      {/* A toolbar, not a PageHeader: the Planning Engine shell already
          renders one above the tab strip. The "read-only" wording is kept
          deliberately — it is the user-facing half of the no-apply-here
          rule in the file header. */}
      <div className="flex items-center gap-2.5">
        <Trophy className="w-5 h-5 shrink-0 text-violet-300" />
        <p className="text-sm text-slate-400">
          Rank saved plans by operating cost + Planning Constraint penalties.{' '}
          <span className="font-semibold text-slate-200">Read-only</span> — apply from the Planning Core tab.
        </p>
      </div>
        ...
        <section className="lg:col-span-2 space-y-3">
          ...
          {loadingPlans ? (
            ...
            <div className="rounded-xl border border-dashed border-slate-700 bg-slate-900/40 p-10 text-center text-slate-400">
              No saved plans yet. Compute + save a couple on the <a href="/bus-ops/plan" className="text-violet-400 hover:underline">Planning Core</a> tab first.
            </div>
          ) : (
            <FleetDataGrid
