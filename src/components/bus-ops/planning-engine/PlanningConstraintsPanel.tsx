...
/**
 * Planning Constraints admin — tab 2 of the Planning Engine.
 *
 * Ops authors PCE rules here. Each row corresponds to one
 * planning_constraints record; the PCE evaluator dispatches on `kind`.
 *
 * Was a standalone page at /bus-ops/planning-constraints, which now
 * redirects here. It sits directly after Operational Rules Engine
 * because visual order follows the data flow — CBA rules and PCE
 * constraints both feed the plan that Planning Core computes.
 *
 * Moving it in also retired PceRulesDrawer, a cut-down editor for the
 * four constraints that bound Planning Core. That drawer existed only
 * because a deep link to the standalone page discarded an in-progress
 * plan. As a tab it cannot: Planning Engine mounts panels lazily and
 * never unmounts them, so Planning Core keeps its state while the
 * operator edits constraints here. One editor instead of two.
 *
...
export interface PlanningConstraintsPanelProps {
  /**
   * Fired after any successful mutation — create, edit, delete, or an
   * inline enable/disable toggle.
   *
   * Planning Core stays mounted on its own tab while constraints are
   * edited here, holding a plan that was computed against the old rules.
   * Without this signal it would keep showing a stale result with no
   * indication anything moved. Mirrors the cbaRevision handshake the CBA
   * tab already uses; like that one, it only offers a recompute — it
   * never silently discards the operator's in-progress work.
   */
  onConstraintsChanged?: () => void;
}

export function PlanningConstraintsPanel({ onConstraintsChanged }: PlanningConstraintsPanelProps = {}) {
  return (
    <RequireTenantAdmin resource="planning-constraints">
      <PlanningConstraintsPanelInner onConstraintsChanged={onConstraintsChanged} />
    </RequireTenantAdmin>
  );
}

function PlanningConstraintsPanelInner({ onConstraintsChanged }: PlanningConstraintsPanelProps) {
  ...
  // Reload the table, then tell the Planning Engine shell the rules moved.
  // Only mutations call this — the initial load above deliberately does not,
  // or simply opening this tab would mark a freshly computed plan stale.
  const reloadAndNotify = useCallback(async () => {
    await load();
    onConstraintsChanged?.();
  }, [load, onConstraintsChanged]);
  ...
  const constraintColumns: DataGridColumn<PlanningConstraint>[] = [
    ...
    { key: 'kind', header: 'Kind', accessor: r => KIND_BY_ID.get(r.kind)?.label ?? r.kind, filter: 'select',
      render: r => (
        <div>
          <div>{KIND_BY_ID.get(r.kind)?.label ?? r.kind}</div>
          <div className="text-xs text-slate-500">{r.kind}</div>
        </div>
      ) },
    { key: 'action', header: 'Action', accessor: r => r.action, filter: 'select',
      render: r => (
    ...
    { key: 'effective', header: 'Effective', accessor: r => r.effectiveFrom,
      render: r => r.effectiveFrom || r.effectiveTo
        ? <span className="text-slate-400">{fmtDate(r.effectiveFrom)} → {fmtDate(r.effectiveTo)}</span>
        : <span className="text-slate-600">always</span> },
    { key: 'enabled', header: 'Enabled', accessor: r => r.isEnabled ? 'Enabled' : 'Disabled', filter: 'select',
      render: r => (
    ...
    { key: 'rowActions', header: '—', align: 'right', filter: false, sortable: false,
      render: r => (
  ...
  return (
    <div className="space-y-8">
      {/* A toolbar, not a PageHeader: the Planning Engine page already
          renders one above the tab strip, and stacking two titles reads as
          a layout bug. The stats line and the actions are kept — they were
          the useful part of the old header. */}
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2.5 min-w-0">
          <Shield className="w-5 h-5 shrink-0 text-violet-300" />
          <p className="text-sm text-slate-400 truncate">
            <span className="font-semibold text-slate-200">{rows.length}</span> defined ·{' '}
            <span className="font-semibold text-slate-200">{enabledCount}</span> enabled ·{' '}
            <span className="font-semibold text-slate-200">{blockCount}</span> BLOCK-active
            <span className="hidden sm:inline"> — evaluated on every plan apply and every merge</span>
          </p>
        </div>
      ...
      {loading && rows.length === 0 ? (
        ...
        <FleetDataGrid
          gridName="PlanningConstraints"
          rows={sortedRows}
          getRowId={r => r.id}
          loading={false}
          emptyMessage="No planning constraints defined yet."
          ...
          toolbar={{
            exportName: 'planning-constraints',
      ...
      {editing !== null && (
        <ConstraintFormModal
          ...
          onSaved={async () => { setEditing(null); await reloadAndNotify(); }}
        />
      )}
    </div>
  );
