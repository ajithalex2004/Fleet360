'use client';
/**
 * CBA / union rule-set management — tab 1 of the Planning Engine.
 *
 * Was the standalone page at /bus-ops/cba-rules, which now redirects to
 * /bus-ops/planning-engine?tab=cba. Moved here rather than left in place
 * because Planning Core already consumes these rule-sets (cbaToWorkRules
 * pre-fills its WorkRules), so editing them used to mean navigating away
 * from a half-built plan.
 *
...
import { Plus, Save, Trash2, X, Star, CheckCircle2, XCircle } from 'lucide-react';
...
export interface CbaRulesPanelProps {
  /**
   * Fired after any mutation that could change what Planning Core would
   * pre-fill into its WorkRules — create, save, delete, or a change of
   * default. The shell uses it to offer a reload rather than silently
   * overwriting a plan the user may already be building on the next tab.
   */
  onRuleSetsChanged?: () => void;
}

export function CbaRulesPanel({ onRuleSetsChanged }: CbaRulesPanelProps = {}) {
  ...
  // Every call site is a post-success refresh (set-default, delete,
  // create, save), so notifying from here covers all four mutation paths
  // without threading the callback through each handler.
  const reload = () => {
    listRes.refresh();
    onRuleSetsChanged?.();
  };
  ...
  return (
    <div className="space-y-6">
      {/* Compact intro rather than a full PageHeader — the Planning Engine
          shell already renders the page title, and a second gradient header
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        ...
        <button onClick={() => setShowNew(true)}
          className="inline-flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-emerald-500/30 hover:opacity-90 shrink-0">
