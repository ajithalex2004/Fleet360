...
export default function ModuleGuard({ moduleId, moduleName, moduleIcon = '🔒', children }: Props) {
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-[var(--bg-canvas)] flex items-center justify-center px-6">
        <div className="max-w-md w-full text-center">
          <div className="w-16 h-16 rounded-2xl bg-[var(--bg-surface-hover)] border border-[var(--border-subtle)] flex items-center justify-center text-3xl mx-auto mb-5">
            {moduleIcon}
          </div>
          <h1 className="text-2xl font-bold text-[var(--text-main)] mb-3">Session required</h1>
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <Link
              href="/platform"
              className="px-6 py-2.5 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface-hover)] text-[var(--text-muted)] text-sm font-medium hover:border-[var(--border-strong)] hover:text-[var(--text-main)] transition-all"
            >
              Back to Platform
            </Link>
          </div>
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-canvas)] flex items-center justify-center px-6">
      <div className="max-w-md w-full text-center">
        {/* Icon */}
        <div className="w-20 h-20 rounded-2xl bg-[var(--bg-surface-hover)] border border-[var(--border-subtle)] flex items-center justify-center text-4xl mx-auto mb-6 grayscale">
          {moduleIcon}
        </div>

        {/* Badge */}
        <div className="inline-flex items-center gap-1.5 rounded-full border border-[var(--border-strong)] bg-[var(--bg-surface-hover)] px-3 py-1 mb-4">
          <span className="text-[var(--text-faint)] text-xs">🔒</span>
          <span className="text-[var(--text-faint)] text-xs font-semibold uppercase tracking-wide">Module Not Subscribed</span>
        </div>
        ...
        <p className="text-[var(--text-muted)] text-sm leading-relaxed mb-2">
          ...
          <strong className="text-[var(--text-muted)]">{displayName}</strong> module.
        </p>
        <p className="text-[var(--text-faint)] text-sm leading-relaxed mb-8">
          Contact your platform administrator to enable this module for your tenant.
        </p>

        {/* Tenant info pill */}
        {tenant && (
          <div className="inline-flex items-center gap-2 bg-[var(--bg-surface-hover)] border border-[var(--border-subtle)] rounded-xl px-4 py-2 mb-8">
            <span className="w-2 h-2 rounded-full bg-blue-400" />
            <span className="text-xs text-[var(--text-muted)]">Signed in as <strong className="text-[var(--text-main)]">{tenant.name}</strong></span>
          </div>
        )}
        ...
        {tenant && tenant.enabledModules.length > 0 && (
          <div className="mt-10 text-left">
            <p className="text-xs text-[var(--text-faint)] font-semibold uppercase tracking-widest mb-3">Your active modules</p>
