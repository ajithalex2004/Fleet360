...
import { X, Upload, FileWarning, FileCheck2, Database, Send } from 'lucide-react';

/**
 * ImportModal
 * ----------------------------------------------------------------------------
 * Mounted from inside /leasing/lessees. Functional port of the legacy
 * /leasing/import page so users no longer need a top-level menu item.
 *
 * Domain model:
 *   - Bulk CSV import for leasing resources. Pick a resource, upload a CSV,
 *     preview the parsed rows (server-side validation, no DB writes), then
 *     commit valid rows. Two resources are currently supported:
 *       * vehicles  — fleet vehicle master records
 *       * lessees   — B2B + B2B customer accounts
 *
 * Pipeline:
 *   Pick resource → upload CSV → Preview (server returns valid/invalid rows
 *   + detected/unknown headers) → Commit (writes valid rows, skips invalid).
 *
 * The component takes only `onClose`. It owns all its own state and
 * talks directly to /api/leasing/import/[resource]. Nothing here talks
 * to Lessees' state.
 * ------------------------------------------------------------------------- */
...
export default function ImportModal({ onClose }: { onClose: () => void }) {
  const [resource, setResource] = useState<Resource>('lessees');
