 ...
 * StaffModal
 * ----------------------------------------------------------------------------
 * Mounted from inside /leasing/drivers. Functional port of the legacy
 * /leasing/staff page so users no longer need a top-level menu item.
 *
 * Domain model:
 *   - Branch staff (people who work on leasing operations): branch managers,
 *     leasing agents, coordinators, drivers (when assigned as staff to a
 *     branch), admins. Each staff member is assigned to one of the leasing
 *     branches across the seven UAE emirates.
 *
 *   - Lifecycle: ACTIVE → ON_LEAVE → ACTIVE / TRANSFERRED → INACTIVE.
 *
 *   - The page's own banner says: "this feature will move to Admin / Branch
 *     Management as Branch Staff Assignments." When that lands, this modal
 *     will be replaced by a button on /admin/branches. For now it's surfaced
 *     on /leasing/drivers as the closest leasing-side home.
 *
 * The component takes only `onClose`. It owns all its own state and hits
 * /api/branch-staff directly.
 * ------------------------------------------------------------------------- */
...
const ROLES = ['BRANCH_MANAGER', 'LEASING_AGENT', 'COORDINATOR', 'DRIVER', 'ADMIN'];

const ROLE_COLORS: Record<string, string> = {
  BRANCH_MANAGER: 'bg-violet-500/20 text-violet-300 border-violet-500/40',
...
export default function StaffModal({ onClose }: { onClose: () => void }) {
  ...
  const fetchStaff = useCallback(async () => {
    setLoading(true);
    try {
      const qs = new URLSearchParams({ module: 'LEASING', limit: '100' });
      ...
      <div className="w-full max-w-7xl max-h-[94vh] flex flex-col bg-slate-800/95 border border-white/10 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-slate-900/60 flex-shrink-0">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => { setEditTarget(null); setForm({ ...EMPTY_FORM }); setError(''); setShowAssign(true); }}
              className="px-3 py-1.5 rounded-lg bg-gradient-to-r from-violet-600 to-purple-600 text-white text-sm font-semibold hover:opacity-90"
            >
              + Assign Staff
            </button>
            <button
                      ...
                      <tr key={s.id} className="hover:bg-white/3 transition-colors">
                        ...
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-1.5">
                            <button onClick={() => openEdit(s)}
                              className="px-2.5 py-1 rounded-lg bg-violet-600/20 border border-violet-500/30 text-violet-400 hover:bg-violet-600/40 text-xs font-medium transition-colors">
                              Edit
                            </button>
                            <button onClick={() => { setTransferTarget(s); setShowTransfer(true); }}
            ...
            <div className="bg-slate-800 border border-white/10 rounded-2xl p-7 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button onClick={handleSave} disabled={saving}
                  className="flex-1 px-4 py-2.5 bg-gradient-to-r from-violet-600 to-purple-600 rounded-xl text-white text-sm font-semibold hover:opacity-90 transition-opacity disabled:opacity-50">
            ...
            <div className="bg-slate-800 border border-white/10 rounded-2xl p-7 w-full max-w-md">
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button onClick={handleTransfer} disabled={saving || !transferForm.branch_name}
                  className="flex-1 px-4 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl text-white text-sm font-semibold hover:opacity-90 transition-opacity disabled:opacity-50">
