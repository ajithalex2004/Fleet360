...
export default function PreTripCheckPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/bus-ops/schedules/${id}/pretrip-check`);
      const data = res.ok ? await res.json() : { checklist: [] };
      setChecklist(data.checklist ?? []);
      // Default everything to ok=true so the driver only flips the failures.
  ...
  const submit = async () => {
    ...
    try {
      ...
      setMsg({
        kind: passed ? 'ok' : 'err',
        text: passed
          ? '✓ Check passed. You can depart from the trip card.'
          : `⛔ UNSAFE — ${data.assessment.blockingFailures.length} blocking failure(s). Departure blocked. Report to dispatch.`,
      });
      if (passed) setTimeout(() => router.push('/bus-ops/driver'), 1500);
    } catch (err) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <p className="text-sm text-slate-400">Walk around the vehicle, tap each item to mark fail. Required before departure.</p>
      </div>

      <div className={`p-3 rounded-xl text-sm ${blockingFailCount > 0 ? 'bg-rose-500/20 border border-rose-500/40 text-rose-200' : failCount > 0 ? 'bg-amber-500/20 border border-amber-500/40 text-amber-200' : 'bg-emerald-500/20 border border-emerald-500/40 text-emerald-200'}`}>
        {blockingFailCount > 0
          ? `${blockingFailCount} blocking failure${blockingFailCount === 1 ? '' : 's'} — departure will be blocked`
          : failCount > 0
            ...
            <div key={c.key} className={`p-3 rounded-xl border ${ok ? 'bg-slate-800/40 border-white/10' : 'bg-rose-500/10 border-rose-500/40'}`}>
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {c.blocking && (
                    <div className="text-[10px] text-amber-300 mt-0.5">⚠ Blocks departure if failed</div>
                  )}
                </div>
                <div className="inline-flex rounded-lg bg-slate-900/60 p-1 shrink-0">
                  <button
                    onClick={() => setOk(c.key, true)}
                    className={`px-3 py-1.5 rounded-md text-xs font-semibold ${ok ? 'bg-emerald-600 text-white' : 'text-slate-400'}`}
                  >
                    OK
                  </button>
                  <button
                    onClick={() => setOk(c.key, false)}
                    className={`px-3 py-1.5 rounded-md text-xs font-semibold ${!ok ? 'bg-rose-600 text-white' : 'text-slate-400'}`}
                  >
                    FAIL
                  </button>
                </div>
      ...
      <div>
        <label className="block text-xs uppercase tracking-wide text-slate-400 mb-1.5 font-semibold">Notes (optional)</label>
        <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={2} className="w-full px-4 py-2 rounded-xl bg-slate-800/60 border border-white/10 text-white" />
      </div>
      ...
      <button
        onClick={submit}
        disabled={busy}
        className="w-full py-3 rounded-xl bg-violet-600 hover:bg-violet-500 text-white font-semibold disabled:opacity-50"
      >
