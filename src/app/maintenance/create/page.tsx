...
export default function CreateRequestPage() {
    ...
    return (
        <div className="space-y-6">
            {/* Header */}
            <div>
                <h1 className="text-2xl font-bold text-slate-900">Create Maintenance Request</h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-8 rounded-xl border border-slate-200 bg-white p-8 shadow-sm">
                ...
                <div>
                    <h3 className="mb-4 text-lg font-semibold text-slate-900">Vehicle Information</h3>
                    <div className="grid gap-6 md:grid-cols-3">
                        <div>
                            ...
                            >
                                <option value="" className="text-slate-900 bg-white">Select Vehicle</option>
                                {vehicles.map((v) => (
                                    <option key={v.id} value={v.id} className="text-slate-900 bg-white">{v.licensePlate} - {v.make} {v.model}</option>
                                ))}
                            </select>
                        </div>
                        <div>
                            ...
                            <input
                                type="text"
                                disabled
                                className="mt-1 block w-full rounded-lg border border-slate-300 bg-slate-50 px-3 py-2 text-slate-500 shadow-sm"
                ...
                <div>
                    <h3 className="mb-4 text-lg font-semibold text-slate-900">Maintenance Details</h3>
                    <div className="grid gap-6 md:grid-cols-2">
                        <div>
                            ...
                            >
                                <option value="" className="text-slate-900 bg-white">Select Type</option>
                    ...
                    {formData.maintenanceType && (
                        <div className="mt-6">
                            ...
                            <div className="max-h-60 overflow-y-auto rounded-lg border border-slate-300 p-3 bg-slate-50">
                                <div className="grid grid-cols-2 gap-2 md:grid-cols-3 lg:grid-cols-4">
                                    {getFilteredJobs().map((job) => (
                                        <label key={job} className="flex items-center space-x-2 rounded-lg border border-slate-200 bg-white p-2 hover:bg-blue-50 cursor-pointer transition-colors">
                                            <input
                                                type="checkbox"
                    ...
                    {formData.maintenanceJobs.length > 0 && (
                        <div className="mt-6">
                            ...
                            {/* Garages List */}
                            <div className="max-h-48 overflow-y-auto rounded-lg border border-slate-300 p-2 bg-white">
                                {getFilteredGarages().map((g) => (
                                    <label key={g.id} className="flex items-start space-x-3 p-3 hover:bg-slate-50 rounded cursor-pointer border-b border-slate-100 last:border-0">
                                        <input
                                            type="checkbox"
                                        ...
                                        <div className="flex-1">
                                            <span className="font-medium text-slate-900 text-sm block">{g.name}</span>
                                            <span className="text-slate-500 text-xs block mt-0.5">{g.location}</span>
                                            <div className="flex flex-wrap gap-1 mt-1">
                                                {(g.specialties || []).map((spec, idx) => (
                                                    <span key={idx} className="inline-flex items-center rounded bg-blue-50 px-1.5 py-0.5 text-xs text-blue-700">
                                                        {spec}
                                                    </span>
                                                ))}
                                            </div>
                    ...
                    <div className="mt-6">
                        <label className="block text-sm font-medium text-slate-700">Work Order No.</label>
                        <input
                            type="text"
                            className="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                ...
                <div>
                    <h3 className="mb-4 text-lg font-semibold text-slate-900">Schedule & Assignment</h3>
                    <div className="grid gap-6 md:grid-cols-3">
                        <div>
                ...
                <div>
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-700">Description / Remarks</label>
                        <textarea
                            rows={4}
                            className="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
