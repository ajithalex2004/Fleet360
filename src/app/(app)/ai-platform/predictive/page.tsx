...
export default function AIPlatformPredictivePage() {
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[var(--text-main)]">Predictive Maintenance</h1>
        <p className="text-xs mt-1 text-[var(--text-faint)]">AI-powered failure prediction and cost forecasting — owned by AI Platform.</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={label} className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] p-5">
            <div className={`rounded-lg bg-${color}-500/20 p-2 w-fit mb-2`}>
              <Icon className={`h-4 w-4 text-${color}-400`} />
            </div>
            <p className="text-xs text-[var(--text-faint)]">{label}</p>
            <p className="text-2xl font-bold text-[var(--text-main)] mt-0.5">{value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface)]">
        <div className="p-5 border-b border-[var(--border-subtle)]">
          <h3 className="font-semibold text-[var(--text-main)]">Failure Predictions</h3>
          <p className="text-sm text-[var(--text-faint)] mt-0.5">Components with predicted failures, ranked by urgency</p>
        </div>
        <div className="divide-y divide-white/10">
          {RISK_ITEMS.map((item, i) => (
            <div key={i} className="flex items-center justify-between p-5 hover:bg-[var(--bg-surface-hover)] transition-colors">
              <div>
                <p className="text-sm font-medium text-[var(--text-main)]">{item.vehicle}</p>
                <p className="text-xs text-[var(--text-faint)] mt-0.5">Component: {item.component}</p>
              </div>
              <div className="flex items-center gap-4 text-right">
                ...
                <div>
                  <p className="text-xs text-[var(--text-faint)]">Confidence</p>
