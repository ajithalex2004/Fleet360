    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-5">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-400">GL Account: 5145 Debit / 4500 Credit</span>
          </div>
          <h1 className="text-2xl font-bold text-white mt-1">Departmental Cost Allocation & Recharge Matrix</h1>
        ...
        <div className="flex items-center flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2 bg-slate-900 border border-slate-800 rounded-lg px-3 py-1.5 text-xs text-slate-200">
            ...
            <select
              ...
              className="bg-transparent text-slate-200 border-none outline-none cursor-pointer font-medium"
            >
              {[
                ...
                <option key={m} value={m} className="bg-slate-900">{name}</option>
              ))}
            </select>
            <select
              value={selectedYear}
          ...
          <button
            ...
            className="flex items-center gap-2 px-3.5 py-1.5 text-xs font-medium text-slate-200 bg-slate-900 hover:bg-slate-800 border border-slate-700 rounded-lg transition disabled:opacity-50"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 px-3.5 py-1.5 text-xs font-semibold text-white bg-emerald-600 hover:bg-emerald-500 rounded-lg shadow-sm transition disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        <div className="bg-slate-900/60 border border-slate-800/80 rounded-xl p-4">
          ...
          <div className="text-xl font-bold text-white">{fmtAed(data?.totalFleetOperatingCost)}</div>
        ...
        <div className="bg-slate-900/60 border border-slate-800/80 rounded-xl p-4">
          ...
          <div className="text-xl font-bold text-white">{data ? data.totalPaxKm.toLocaleString('en-AE') : '—'}</div>
          <p className="text-[11px] text-slate-500 mt-2">Cumulative passenger distance</p>
        </div>
        ...
        <div className="bg-slate-900/60 border border-slate-800/80 rounded-xl p-4">
          ...
          <p className="text-[11px] text-slate-500 mt-2">True transport efficiency</p>
        </div>
        ...
        <div className="bg-slate-900/60 border border-slate-800/80 rounded-xl p-4">
          ...
          <div className="text-xl font-bold text-white">{data ? data.totalBoardings.toLocaleString('en-AE') : '—'}</div>
          <p className="text-[11px] text-slate-500 mt-2">Avg {fmtAed(data?.costPerBoarding)} / scan</p>
        </div>
        ...
        <div className="bg-slate-900/60 border border-slate-800/80 rounded-xl p-4">
          ...
          <p className="text-[11px] text-slate-500 mt-2">{data?.departments.length || 0} cost centers billed</p>
        </div>
      </div>
      ...
      {data && data.departments.length > 0 && (
        <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-4 space-y-3">
          <div className="flex justify-between items-center text-xs">
            <span className="font-semibold text-slate-200">Departmental Pax-Km Share Distribution</span>
            <span className="text-slate-400">{data.totalPaxKm.toLocaleString('en-AE')} Total Pax-Km</span>
          </div>
          ...
          <div className="flex flex-wrap gap-3 pt-1">
            {data.departments.slice(0, 6).map((d, i) => {
              ...
              const color = colors[i % colors.length];
              return (
                <div key={d.departmentName} className="flex items-center gap-1.5 text-xs text-slate-400">
      ...
      <div className="bg-slate-900/50 border border-slate-800 rounded-xl overflow-hidden shadow-sm">
        <div className="p-4 border-b border-slate-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <div>
            <h3 className="text-sm font-bold text-white">Department Recharge Matrix — {data?.period}</h3>
          ...
          <input
            ...
            className="bg-slate-950 border border-slate-800 rounded-lg px-3 py-1.5 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-indigo-500 w-full sm:w-64"
          />
        </div>
          ...
          <table className="w-full text-left text-xs text-slate-300">
            ...
            <tbody className="divide-y divide-slate-800/60">
              {loading ? (
                ...
                filteredDepts.map((d) => (
                  <tr key={d.departmentName} className="hover:bg-slate-800/40 transition">
                    <td className="py-3 px-4">
                      <div className="font-semibold text-slate-100">{d.departmentName}</div>
                    ...
                    <td className="py-3 px-4 text-center">
                      <span className="font-medium text-slate-200">{d.activeRiders}</span>
                    </td>
            ...
            {data && data.departments.length > 0 && (
              <tfoot className="bg-slate-900 font-semibold text-slate-200 border-t-2 border-slate-700">
                <tr>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl">
            <div className="flex items-start justify-between">
              <div>
                ...
                <h3 className="text-lg font-bold text-white mt-1">Configure Unit Chargeback Rates</h3>
              </div>
              <button onClick={() => setShowRateModal(false)} className="text-slate-500 hover:text-slate-300 text-lg leading-none">
                ✕
              </button>
            </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl">
            <div className="flex items-start justify-between">
              <div>
                ...
                <h3 className="text-lg font-bold text-white mt-1">Post Recharge Journal Entry</h3>
              </div>
              <button onClick={() => setShowGlModal(false)} className="text-slate-500 hover:text-slate-300 text-lg leading-none">
                ✕
              </button>
            </div>
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 text-xs font-semibold text-white bg-emerald-600 hover:bg-emerald-500 rounded-xl shadow transition disabled:opacity-50"
              >
