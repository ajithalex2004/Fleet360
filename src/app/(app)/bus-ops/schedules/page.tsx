...
export default function SchedulesPage() {
  ...
  const scheduleColumns: DataGridColumn<Schedule>[] = useMemo(() => [
    ...
    },
    {
      ...
      render: (s) => (
        <div>
          <div className="font-medium text-[var(--text-main)]">{s.route?.name ?? '—'}</div>
          <div className="text-xs text-[var(--text-faint)]">{s.route?.origin} → {s.route?.destination}</div>
        </div>
      ),
    },
    ...
    },
    {
      ...
      render: (s) => {
        ...
        if (!actions.length) return <span className="text-[var(--text-faint)] text-xs">—</span>;
        return <RowActionsMenu actions={actions} />;
      },
    },
  ], [selectedIds, actionLoading]);

  if (loading) return <div className="flex items-center justify-center h-full"><div className="text-[var(--text-muted)] animate-pulse">Loading schedules...</div></div>;
    ...
    <div className="space-y-8">
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            {selectedIds.size > 0 && (
              <button onClick={clearSelection} className="text-sm text-[var(--text-muted)] hover:text-[var(--text-main)]">Clear</button>
            )}
      ...
      <div className="flex gap-4 flex-wrap">
        ...
        <input type="date" value={dateFilter} onChange={e=>setDate(e.target.value)}
          className="px-4 py-2 rounded-lg bg-[var(--bg-surface)]/50 border border-[var(--border-subtle)] text-[var(--text-main)] focus:border-violet-500 focus:outline-none" />
        {dateFilter && <button onClick={()=>setDate('')} className="text-sm text-[var(--text-muted)] hover:text-[var(--text-main)]">Clear date</button>}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-[var(--bg-surface)]/95 border border-[var(--border-subtle)] rounded-2xl p-8">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-[var(--text-main)]">New Trip Schedule</h2>
              <button onClick={()=>setShowModal(false)} className="text-[var(--text-muted)] hover:text-[var(--text-main)]">✕</button>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className="block text-sm font-medium text-[var(--text-muted)] mb-2">Route *</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-[var(--text-muted)] mb-2">Departure Time *</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-[var(--text-muted)] mb-2">Shift Type</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-[var(--text-muted)] mb-2">Direction</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-[var(--text-muted)] mb-2">Capacity</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-[var(--text-muted)] mb-2">Assign Vehicle</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-[var(--text-muted)] mb-2">Assign Driver</label>
