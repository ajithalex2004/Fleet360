  ...
  if (loading) {
    return (
      <div className="space-y-4 animate-pulse">
        <div className="h-10 w-72 bg-[var(--bg-surface)] rounded-xl" />
        <div className="grid grid-cols-5 gap-4">{[...Array(5)].map((_,i) => <div key={i} className="h-24 bg-[var(--bg-surface)] rounded-2xl" />)}</div>
        <div className="grid grid-cols-2 gap-4">{[...Array(4)].map((_,i) => <div key={i} className="h-56 bg-[var(--bg-surface)] rounded-2xl" />)}</div>
      </div>
    );
  }

  if (!data) return <div className="text-[var(--text-muted)] text-center py-20">No analytics data available.</div>;
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        accent="violet"
        actions={
          <button onClick={load} className="inline-flex items-center gap-1.5 text-xs text-[var(--text-muted)] border border-[var(--border-subtle)] px-3 py-1.5 rounded-lg hover:border-[var(--border-strong)] hover:bg-[var(--bg-surface-hover)] transition-colors">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <KpiCard icon="🚌" label="Total Trips"     value={kpis.totalTrips}      color="text-[var(--text-main)]" />
      ...
      {/* Performance bars */}
      <div className="bg-[var(--bg-surface)]/60 border border-[var(--border-subtle)] rounded-2xl p-5 grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <Link href="/sustainability/fleet-carbon"
          ...
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-600 to-teal-600 flex items-center justify-center shadow-lg shadow-emerald-500/20 shrink-0">
            <Leaf className="w-5 h-5 text-[var(--text-main)]" strokeWidth={1.75} />
          </div>
          <div className="min-w-0">
            ...
            <div className="text-sm font-semibold text-emerald-950 dark:text-[var(--text-main)] mt-0.5">Fleet Carbon Dashboard</div>
        ...
        <Link href="/maintenance/action-centre"
          ...
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-cyan-600 to-blue-600 flex items-center justify-center shadow-lg shadow-cyan-500/20 shrink-0">
            <Wrench className="w-5 h-5 text-[var(--text-main)]" strokeWidth={1.75} />
          </div>
          <div className="min-w-0">
            <div className="text-[10px] uppercase tracking-wider font-semibold text-cyan-800 dark:text-cyan-200">Maintenance</div>
            <div className="text-sm font-semibold text-cyan-950 dark:text-[var(--text-main)] mt-0.5">Service Action Centre</div>
        ...
        <Link href="/compliance/salik"
          ...
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-amber-600 to-orange-600 flex items-center justify-center shadow-lg shadow-amber-500/20 shrink-0">
            <ShieldCheck className="w-5 h-5 text-[var(--text-main)]" strokeWidth={1.75} />
          </div>
          <div className="min-w-0">
            <div className="text-[10px] uppercase tracking-wider font-semibold text-amber-800 dark:text-amber-200">Compliance</div>
            <div className="text-sm font-semibold text-amber-950 dark:text-[var(--text-main)] mt-0.5">Salik / Toll Accounts</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-[var(--bg-surface)]/60 border border-[var(--border-subtle)] rounded-2xl p-5 space-y-4">
          <p className="text-xs text-[var(--text-faint)] uppercase tracking-wider font-medium">On-time SLA (last 30 days)</p>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-3xl font-bold text-emerald-400">{kpis.onTimeDeparturePct ?? 0}%</p>
              <p className="text-xs text-[var(--text-faint)] mt-0.5">Departure ≤ +5 min</p>
              <p className="text-[10px] text-[var(--text-faint)] mt-1">avg delay {kpis.avgDepartureDelayMin ?? 0} min</p>
            </div>
            <div>
              <p className="text-3xl font-bold text-cyan-400">{kpis.onTimeArrivalPct ?? 0}%</p>
              <p className="text-xs text-[var(--text-faint)] mt-0.5">Arrival ≤ +10 min</p>
              <p className="text-[10px] text-[var(--text-faint)] mt-1">avg delay {kpis.avgArrivalDelayMin ?? 0} min</p>
            </div>
          </div>
          {(charts.slaByRoute ?? []).length > 0 && (
            <div className="border-t border-[var(--border-subtle)] pt-3 space-y-1.5">
              <p className="text-[10px] text-[var(--text-faint)] uppercase">Worst on-time routes</p>
              {charts.slaByRoute!.map(r => (
                <div key={r.name} className="flex items-center justify-between text-xs">
                  <span className="text-[var(--text-muted)] truncate">{r.name}</span>
        ...
        <div className="bg-[var(--bg-surface)]/60 border border-[var(--border-subtle)] rounded-2xl p-5 space-y-4">
          <div className="flex items-start justify-between">
            <p className="text-xs text-[var(--text-faint)] uppercase tracking-wider font-medium">Cost (last 30 days)</p>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <p className="text-2xl font-bold text-[var(--text-main)]">AED {(kpis.costPerTrip ?? 0).toLocaleString('en-US', { maximumFractionDigits: 0 })}</p>
              <p className="text-xs text-[var(--text-faint)] mt-0.5">per trip</p>
            </div>
            <div>
              <p className="text-2xl font-bold text-[var(--text-main)]">AED {(kpis.costPerPassenger ?? 0).toFixed(2)}</p>
              <p className="text-xs text-[var(--text-faint)] mt-0.5">per passenger</p>
            </div>
            <div>
              <p className="text-2xl font-bold text-[var(--text-main)]">AED {(kpis.costPerKm ?? 0).toFixed(2)}</p>
              <p className="text-xs text-[var(--text-faint)] mt-0.5">per km</p>
            </div>
          </div>
          {kpis.costBreakdown && (
            <div className="border-t border-[var(--border-subtle)] pt-3 space-y-1.5 text-xs">
              <p className="text-[10px] text-[var(--text-faint)] uppercase">Breakdown · total AED {(kpis.totalCost ?? 0).toLocaleString('en-US', { maximumFractionDigits: 0 })}</p>
              <div className="flex items-center justify-between"><span className="text-[var(--text-muted)]">⛽ Fuel</span><span className="text-amber-300">AED {kpis.costBreakdown.fuel.toLocaleString()}</span></div>
              <div className="flex items-center justify-between"><span className="text-[var(--text-muted)]">🧑‍✈️ Driver</span><span className="text-cyan-300">AED {kpis.costBreakdown.driver.toLocaleString()}</span></div>
              <div className="flex items-center justify-between"><span className="text-[var(--text-muted)]">🚌 Vehicle</span><span className="text-violet-300">AED {kpis.costBreakdown.vehicle.toLocaleString()}</span></div>
            </div>
          )}
      ...
      {(charts.boardingMethods ?? []).length > 0 && (
        <div className="bg-[var(--bg-surface)]/60 border border-[var(--border-subtle)] rounded-2xl p-5">
          <p className="text-xs text-[var(--text-faint)] uppercase tracking-wider font-medium mb-3">Boarding method adoption (last 30 days)</p>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            {(charts.boardingMethods ?? []).map(m => {
              const total = charts.boardingMethods!.reduce((s, x) => s + x.count, 0);
              const pct = total > 0 ? Math.round((m.count / total) * 100) : 0;
              return (
                <div key={m.method} className="rounded-xl bg-[var(--bg-surface)]/60 border border-[var(--border-subtle)] p-3 text-center">
                  <p className="text-2xl font-bold text-[var(--text-main)]">{m.count}</p>
                  <p className="text-xs text-[var(--text-faint)] mt-0.5">{m.method.replace('_', ' ')}</p>
                  <p className="text-[10px] text-[var(--text-faint)] mt-1">{pct}%</p>
                </div>
              );
            })}
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-[var(--bg-surface)]/60 border border-[var(--border-subtle)] rounded-2xl p-5">
          <p className="text-xs text-[var(--text-faint)] uppercase tracking-wider font-medium mb-4">Daily Trips & Passengers (14 days)</p>
        ...
        <div className="bg-[var(--bg-surface)]/60 border border-[var(--border-subtle)] rounded-2xl p-5">
          <p className="text-xs text-[var(--text-faint)] uppercase tracking-wider font-medium mb-4">Peak Hours Distribution</p>
          <ResponsiveContainer width="100%" height={200}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="bg-[var(--bg-surface)]/60 border border-[var(--border-subtle)] rounded-2xl p-5 lg:col-span-2">
          <p className="text-xs text-[var(--text-faint)] uppercase tracking-wider font-medium mb-4">Top Routes by Trips & Passengers</p>
          <ResponsiveContainer width="100%" height={200}>
        ...
        <div className="bg-[var(--bg-surface)]/60 border border-[var(--border-subtle)] rounded-2xl p-5">
          <p className="text-xs text-[var(--text-faint)] uppercase tracking-wider font-medium mb-4">Trips by Shift</p>
          ...
          <div className="flex flex-wrap gap-2 justify-center mt-2">
            {charts.byShift.map((s, i) => (
              <div key={s.name} className="flex items-center gap-1 text-xs text-[var(--text-muted)]">
      ...
      {costDrill && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="w-full max-w-6xl max-h-[85vh] bg-[var(--bg-surface)]/95 border border-[var(--border-subtle)] rounded-2xl flex flex-col">
            <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--border-subtle)]">
              <div>
                <h2 className="text-lg font-bold text-[var(--text-main)]">Cost breakdown (last 30 days)</h2>
                <div className="flex gap-2 mt-2">
                  <button onClick={() => setCostDrill('trip')}
                    className={`text-[11px] px-3 py-1 rounded-full border ${costDrill === 'trip' ? 'bg-violet-500/25 border-violet-500/50 text-[var(--text-main)]' : 'border-[var(--border-subtle)] text-[var(--text-muted)] hover:border-[var(--border-strong)]'}`}>
                    Per Trip
                  </button>
                  <button onClick={() => setCostDrill('route')}
                    className={`text-[11px] px-3 py-1 rounded-full border ${costDrill === 'route' ? 'bg-violet-500/25 border-violet-500/50 text-[var(--text-main)]' : 'border-[var(--border-subtle)] text-[var(--text-muted)] hover:border-[var(--border-strong)]'}`}>
                    Per Route
                  </button>
                </div>
              </div>
              <button onClick={() => setCostDrill(null)} className="text-[var(--text-muted)] hover:text-[var(--text-main)]">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-5">
              {costLoading ? (
                <div className="text-center text-[var(--text-muted)] text-sm animate-pulse py-10">Loading breakdown…</div>
                ...
                <>
                  <div className="text-[10px] text-[var(--text-faint)] mb-3 uppercase">
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-[var(--border-subtle)] text-xs text-[var(--text-muted)]">
                        ...
                        {costDrill === 'route' && <th className="px-3 py-2 text-right text-[var(--text-muted)]">/ Trip</th>}
                        <th className="px-3 py-2 text-right text-[var(--text-muted)]">/ Pax</th>
                      </tr>
                    </thead>
                    <tbody>
                      {costResp.rows.map(r => (
                        <tr key={r.id} className="border-b border-[var(--border-subtle)] hover:bg-[var(--bg-surface-hover)]">
                          <td className="px-3 py-2 font-mono text-xs text-[var(--text-main)]">{r.label}</td>
                          {costDrill === 'trip'
                            ? <><td className="px-3 py-2 text-xs text-[var(--text-muted)] truncate max-w-[10rem]">{r.routeName ?? '—'}</td>
                                <td className="px-3 py-2 text-xs text-[var(--text-muted)] whitespace-nowrap">{r.departureAt ? new Date(r.departureAt).toLocaleDateString('en-AE', { day:'2-digit', month:'short'}) : '—'}</td></>
                            : <td className="px-3 py-2 text-xs text-right text-[var(--text-muted)]">{r.trips ?? '—'}</td>}
                          <td className="px-3 py-2 text-xs text-right text-[var(--text-main)] font-mono">{r.fuelL}</td>
                          <td className="px-3 py-2 text-xs text-right text-[var(--text-main)] font-mono">{r.km}</td>
                          <td className="px-3 py-2 text-xs text-right text-[var(--text-main)] font-mono">{r.minutes}</td>
                          <td className="px-3 py-2 text-xs text-right text-[var(--text-main)] font-mono">{r.passengers}</td>
                          ...
                          <td className="px-3 py-2 text-xs text-right text-[var(--text-main)] font-bold font-mono">{r.totalCost.toLocaleString('en-US', { maximumFractionDigits: 0 })}</td>
                          {costDrill === 'route' && <td className="px-3 py-2 text-xs text-right text-[var(--text-muted)] font-mono">{r.costPerTrip != null ? r.costPerTrip.toFixed(0) : '—'}</td>}
                          <td className="px-3 py-2 text-xs text-right text-[var(--text-muted)] font-mono">{r.costPerPax != null ? r.costPerPax.toFixed(2) : '—'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
