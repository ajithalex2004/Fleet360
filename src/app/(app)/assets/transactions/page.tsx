...
const TYPE_BADGES: Record<string, string> = {
  ...
  WRITE_OFF: 'bg-[var(--bg-surface-hover)] text-[var(--text-muted)] border-slate-500',
};
...
export default function TransactionLedgerPage() {
  ...
  if (loading) return (
    <div className="p-8 space-y-4">
      ...
      {Array.from({ length: 8 }).map((_, i) => <div key={i} className="h-10 bg-[var(--bg-surface)] rounded animate-pulse" />)}
    </div>
  );
  ...
  return (
    <div className="p-8 space-y-5">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-[var(--text-main)]">Transaction Ledger</h1><p className="text-[var(--text-muted)] text-sm">Full audit trail of all stock movements</p></div>
        <button onClick={exportCSV} className="flex items-center gap-2 bg-[var(--bg-surface)] border border-[var(--border-subtle)] hover:bg-[var(--bg-surface-hover)] text-[var(--text-muted)] px-4 py-2 rounded-lg text-sm">⬇ Export CSV</button>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-lg px-3 py-2 text-sm text-[var(--text-main)]" />
        <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-lg px-3 py-2 text-sm text-[var(--text-main)]" />
        ...
        <input value={refSearch} onChange={e => setRefSearch(e.target.value)} placeholder="Search reference no..." className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-lg px-3 py-2 text-sm text-[var(--text-main)] w-48 placeholder-[var(--text-faint)]" />
      </div>
      ...
      <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-xl overflow-hidden">
            ...
            <tbody className="divide-y divide-white/5">
              {filtered.length === 0 ? (
                ...
                <tr key={t.id} className="hover:bg-[var(--bg-surface-hover)] transition-colors">
                  <td className="px-4 py-3 text-[var(--text-faint)] text-xs">{idx + 1}</td>
                  <td className="px-4 py-3 text-[var(--text-muted)] text-xs whitespace-nowrap">{new Date(t.created_at).toLocaleString('en-AE', { dateStyle: 'short', timeStyle: 'short' })}</td>
                  <td className="px-4 py-3">
                    <div className="text-[var(--text-main)] text-xs font-medium">{t.asset_name ?? '—'}</div>
                    {t.asset_no && <div className="text-[var(--text-faint)] text-xs font-mono">{t.asset_no}</div>}
                  </td>
                  <td className="px-4 py-3">
                    <span className={`inline-block px-2 py-0.5 rounded-full text-xs border ${TYPE_BADGES[t.transaction_type] ?? 'bg-[var(--bg-surface-hover)] text-[var(--text-muted)]'}`}>
                      {t.transaction_type.replace('_', ' ')}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-[var(--text-muted)]">{t.quantity_before}</td>
                  <td className="px-4 py-3">
                    <span className={`font-semibold ${t.quantity_change > 0 ? 'text-emerald-400' : t.quantity_change < 0 ? 'text-red-400' : 'text-[var(--text-muted)]'}`}>
                      {t.quantity_change > 0 ? '+' : ''}{t.quantity_change}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-[var(--text-muted)] font-medium">{t.quantity_after}</td>
                  <td className="px-4 py-3 text-[var(--text-muted)] font-mono text-xs">{t.reference_no ?? '—'}</td>
                  <td className="px-4 py-3 text-yellow-300">{t.value_aed ? t.value_aed.toLocaleString() : '—'}</td>
                  <td className="px-4 py-3 text-[var(--text-muted)] text-xs">{t.performed_by ?? '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="px-4 py-3 border-t border-[var(--border-subtle)] flex items-center justify-between">
          <span className="text-[var(--text-faint)] text-sm">{filtered.length} transactions shown</span>
          <div className="flex items-center gap-3">
            <span className="text-[var(--text-muted)] text-sm">Running Total Value:</span>
