  ...
  return (
    <div className="p-8 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-[var(--text-main)]">Medical Assets</h1><p className="text-[var(--text-muted)] text-sm">Controlled substance and medical supply tracking</p></div>
        <button onClick={openAdd} className="bg-yellow-400 hover:bg-yellow-300 text-white font-semibold px-4 py-2 rounded-lg text-sm">+ Add Medical Asset</button>
      </div>
              ...
              {items.length === 0 ? (
                ...
                return (
                  <tr key={m.id} className="hover:bg-[var(--bg-surface-hover)] transition-colors">
                    ...
                    <td className="px-3 py-3 text-[var(--text-main)] font-medium">
                      {m.is_restricted && <span className="text-red-400 mr-1">🔒</span>}
                      {m.name}
                    </td>
                    <td className="px-3 py-3 text-[var(--text-muted)]">{m.asset_type ?? '—'}</td>
                    <td className="px-3 py-3 text-[var(--text-muted)] font-mono text-xs">{m.batch_number ?? '—'}</td>
                    <td className="px-3 py-3">
                      {expired
                        ...
                        : <span className="text-[var(--text-muted)] text-xs">{m.expiry_date ? new Date(m.expiry_date).toLocaleDateString() : '—'}</span>}
                    </td>
                    <td className="px-3 py-3 text-[var(--text-muted)] font-medium">{m.quantity}</td>
                    <td className="px-3 py-3 text-[var(--text-muted)]">{m.unit ?? '—'}</td>
                    <td className="px-3 py-3 text-[var(--text-muted)] font-mono text-xs">{m.current_seal_no ?? '—'}</td>
                    <td className="px-3 py-3 text-[var(--text-muted)]">{m.storage_location ?? '—'}</td>
                    <td className="px-3 py-3 text-[var(--text-muted)]">{m.domain ?? '—'}</td>
                    <td className="px-3 py-3">
                      {m.variance !== 0 && m.variance !== undefined && (
                        <span className="text-amber-400 text-xs mr-1">⚠️</span>
                      )}
                      <span className={`inline-block px-2 py-0.5 rounded-full text-xs border ${m.status === 'ACTIVE' ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' : 'bg-[var(--bg-surface-hover)] text-[var(--text-muted)] border-[var(--border-strong)]'}`}>
                        {m.status ?? 'ACTIVE'}
                      </span>
                    </td>
                    <td className="px-3 py-3">
                      <button onClick={() => openEdit(m)} className="text-xs bg-[var(--bg-surface-hover)] hover:bg-[var(--bg-surface-hover)] text-[var(--text-muted)] px-2 py-1 rounded">Edit</button>
                    </td>
                  </tr>
                );
              })}
        ...
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between p-5 border-b border-[var(--border-subtle)] flex-shrink-0">
              <h2 className="text-[var(--text-main)] font-semibold">{editItem ? 'Edit Medical Asset' : 'Add Medical Asset'}</h2>
            ...
            <div className="flex-1 overflow-y-auto p-5">
              {tab === 0 && (
                <div className="grid grid-cols-2 gap-4">
                  ...
                  {F('category', 'Category')}
                  <div>
                    <label className="block text-xs text-[var(--text-muted)] mb-1">Restricted</label>
                    <label className="flex items-center gap-2 mt-2">
                      <input type="checkbox" checked={form.is_restricted ?? false} onChange={e => setForm(p => ({ ...p, is_restricted: e.target.checked }))} />
                      <span className="text-sm text-[var(--text-muted)]">Is Restricted / Controlled</span>
                    </label>
                  </div>
                  ...
                  <div>
                    ...
                    <select value={form.domain ?? 'AMBULANCE'} onChange={e => setForm(p => ({ ...p, domain: e.target.value }))} className="w-full bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-lg px-3 py-2 text-sm text-[var(--text-main)]">
              ...
              {tab === 1 && editItem && (
                <div className="space-y-6">
                  <div className="bg-[var(--bg-surface)]/50 rounded-xl p-4 space-y-2 text-sm">
                    <div className="flex justify-between"><span className="text-[var(--text-muted)]">Current Seal</span><span className="text-[var(--text-main)] font-mono">{editItem.current_seal_no ?? 'None'}</span></div>
                    <div className="flex justify-between"><span className="text-[var(--text-muted)]">Last Sealed By</span><span className="text-[var(--text-muted)]">{editItem.last_sealed_by ?? '—'}</span></div>
                    <div className="flex justify-between"><span className="text-[var(--text-muted)]">Last Sealed At</span><span className="text-[var(--text-muted)]">{editItem.last_sealed_at ? new Date(editItem.last_sealed_at).toLocaleString() : '—'}</span></div>
                  </div>
                  <div>
                    <label className="block text-xs text-[var(--text-muted)] mb-1">Seal Number (for Seal action)</label>
                    <input value={sealInput} onChange={e => setSealInput(e.target.value)} placeholder="e.g. SEAL-001" className="w-full bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-lg px-3 py-2 text-sm text-[var(--text-main)] mb-3" />
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => sealAction('unseal')} className="flex items-center gap-1.5 bg-[var(--bg-surface-hover)] hover:bg-[var(--bg-surface-hover)] text-[var(--text-main)] px-3 py-1.5 rounded-lg text-sm">🔓 Unseal</button>
                  ...
                  <div className="border-t border-[var(--border-subtle)] pt-4">
                    <h3 className="text-[var(--text-main)] font-medium mb-3 text-sm">Stock Count</h3>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-xs text-[var(--text-muted)] mb-1">Count Date</label>
                        <input type="date" value={stockCount.date} onChange={e => setStockCount(p => ({ ...p, date: e.target.value }))} className="w-full bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-lg px-3 py-2 text-sm text-[var(--text-main)]" />
                      </div>
                      <div>
                        <label className="block text-xs text-[var(--text-muted)] mb-1">Actual Quantity</label>
                        <input type="number" value={stockCount.qty} onChange={e => setStockCount(p => ({ ...p, qty: parseFloat(e.target.value) || 0 }))} className="w-full bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-lg px-3 py-2 text-sm text-[var(--text-main)]" />
                      </div>
                    </div>
                    ...
                    <button onClick={submitStockCount} disabled={submitting} className="mt-3 bg-yellow-400 hover:bg-yellow-300 text-white font-semibold px-4 py-2 rounded-lg text-sm disabled:opacity-50">Record Count</button>
                  </div>
                </div>
              )}
              {tab === 1 && !editItem && (
                <p className="text-[var(--text-faint)] text-sm">Save the asset first to manage seals and stock counts.</p>
              )}
              {tab === 2 && editItem && (
                <div className="space-y-3">
                  {(!editItem.seal_logs || editItem.seal_logs.length === 0) ? (
                    <div className="text-center text-[var(--text-faint)] py-8">
                    ...
                    <div key={l.id ?? i} className="flex gap-3 items-start">
                      <div className="w-8 h-8 rounded-full bg-[var(--bg-surface-hover)] flex items-center justify-center text-xs flex-shrink-0">{l.action === 'SEAL' ? '🔒' : l.action === 'UNSEAL' ? '🔓' : '✓'}</div>
                      <div className="flex-1 bg-[var(--bg-surface)]/50 rounded-xl p-3 text-sm">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-[var(--text-main)] font-medium">{l.action}</span>
                          <span className="text-[var(--text-faint)] text-xs">{new Date(l.performed_at).toLocaleString()}</span>
                        </div>
                        <p className="text-[var(--text-muted)]">By: {l.performed_by}</p>
                        {l.seal_no && <p className="text-[var(--text-muted)]">Seal: <span className="font-mono text-[var(--text-muted)]">{l.seal_no}</span></p>}
                        {l.notes && <p className="text-[var(--text-muted)] mt-1">{l.notes}</p>}
                      </div>
                    </div>
                  ))}
                </div>
              )}
              {tab === 2 && !editItem && <p className="text-[var(--text-faint)] text-sm">Save the asset first to see seal logs.</p>}
            </div>
