  ...
  return (
    <div className="p-8 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-[var(--text-main)]">Return Requests</h1><p className="text-[var(--text-muted)] text-sm">Reverse logistics and stock restoration</p></div>
        <button onClick={() => { setShowCreate(true); setSelected(null); }} className="bg-yellow-400 hover:bg-yellow-300 text-white font-semibold px-4 py-2 rounded-lg text-sm">+ New Return</button>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Left: Return List */}
        <div className="space-y-4">
          <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="w-full bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-lg px-3 py-2 text-sm text-[var(--text-main)]">
          ...
          {filtered.length === 0 ? (
            ...
            <button key={r.id} onClick={() => { setSelected(r); setShowCreate(false); }}
              className={`w-full text-left bg-[var(--bg-surface)] border rounded-xl p-4 transition-all ${selected?.id === r.id ? 'border-yellow-500/50 bg-yellow-500/5' : 'border-[var(--border-subtle)] hover:border-[var(--border-strong)]'}`}>
              <div className="flex items-start justify-between mb-2">
                <div>
                  <p className="text-yellow-300 font-mono text-xs">{r.return_no}</p>
                  <p className="text-[var(--text-main)] font-semibold text-sm">{r.technician_name}</p>
                </div>
        ...
        <div>
          {showCreate && (
            <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-xl p-5 space-y-4">
              <h2 className="text-[var(--text-main)] font-semibold">New Return Request</h2>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="text-xs text-[var(--text-muted)] font-medium">Return Items</label>
                  <button onClick={addItem} className="text-xs text-yellow-400 hover:text-yellow-300">+ Add Item</button>
                </div>
                <div className="space-y-3">
                  {form.items.map((item, i) => (
                    <div key={i} className="bg-[var(--bg-surface)]/50 rounded-xl p-3 space-y-2">
                      <div className="flex gap-2 items-start">
                        <div className="flex-1">
                          ...
                          }} className="w-full bg-[var(--bg-surface-hover)] border border-[var(--border-subtle)] rounded-lg px-2 py-1.5 text-xs text-[var(--text-main)]">
                            <option value="">— Select Asset —</option>
                            {assets.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}
                          </select>
                        </div>
                        <div className="w-16">
                          <input type="number" min={1} value={item.quantity} onChange={e => setItem(i, 'quantity', parseInt(e.target.value) || 1)} className="w-full bg-[var(--bg-surface-hover)] border border-[var(--border-subtle)] rounded-lg px-2 py-1.5 text-xs text-[var(--text-main)] text-center" placeholder="Qty" />
                        </div>
                        {form.items.length > 1 && (
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          <label className="block text-xs text-[var(--text-faint)] mb-1">Condition</label>
                          <select value={item.condition} onChange={e => setItem(i, 'condition', e.target.value)} className="w-full bg-[var(--bg-surface-hover)] border border-[var(--border-subtle)] rounded-lg px-2 py-1.5 text-xs text-[var(--text-main)]">
                            {CONDITIONS.map(c => <option key={c}>{c}</option>)}
                          </select>
                        </div>
                        <div>
                          <label className="block text-xs text-[var(--text-faint)] mb-1">Reason</label>
                          <input value={item.reason ?? ''} onChange={e => setItem(i, 'reason', e.target.value)} className="w-full bg-[var(--bg-surface-hover)] border border-[var(--border-subtle)] rounded-lg px-2 py-1.5 text-xs text-[var(--text-main)]" placeholder="Optional" />
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          {selected && !showCreate && (
            <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-xl p-5 space-y-5">
              ...
              {selected.requested_at && (
                <p className="text-[var(--text-muted)] text-xs">Requested: {new Date(selected.requested_at).toLocaleString()}</p>
              )}

              {/* Items Table */}
              <div>
                <h3 className="text-[var(--text-main)] font-medium text-sm mb-2">Items to Return</h3>
                  ...
                  <tbody className="divide-y divide-white/5">
                    {(selected.items ?? []).map((it, i) => (
                      <tr key={i} className="text-[var(--text-muted)]">
                        ...
                        <td className="py-2 text-[var(--text-muted)]">{it.reason ?? '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              {selected.notes && (
                <div className="bg-[var(--bg-surface)]/50 rounded-xl p-3 text-xs text-[var(--text-muted)] italic">{selected.notes}</div>
              )}
