...
const DOMAIN_COLORS: Record<string, string> = {
  ...
  GENERAL: 'bg-[var(--bg-surface-hover)]/40 text-[var(--text-muted)] border-slate-500/25',
  ...
  return (
    <div className="p-8 space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[var(--text-main)]">🗺️ Zone Authorization Rules</h1>
          <p className="text-[var(--text-muted)] text-xs mt-1">Define which asset domains are authorized in each gateway zone</p>
        </div>
        <button
          onClick={openModal}
          className="bg-yellow-400 hover:bg-yellow-300 text-white font-semibold px-4 py-2 rounded-lg text-sm"
        >
          ⊕ New Rule
        </button>
      </div>
      ...
      {/* Filter */}
      <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-xl p-4 flex items-end gap-4">
        <div className="flex-1 max-w-xs">
          <label className="block text-xs text-[var(--text-muted)] mb-1">Filter by Gateway</label>
          <select
            value={gatewayFilter}
        ...
        <div className="text-xs text-[var(--text-faint)]">{rules.length} rule{rules.length !== 1 ? 's' : ''} total</div>
      </div>
      ...
      {loading ? (
        <div className="space-y-4">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-xl overflow-hidden">
              <div className="h-12 bg-[var(--bg-surface)]/50 animate-pulse" />
              {Array.from({ length: 2 }).map((_, j) => (
                <div key={j} className="h-16 bg-[var(--bg-surface)] border-t border-[var(--border-subtle)] animate-pulse" />
              ))}
            </div>
          ))}
        </div>
      ) : rules.length === 0 ? (
        <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-xl p-16 text-center text-[var(--text-faint)]">
          <div className="text-4xl mb-3">🗺️</div>
          <p className="font-medium text-[var(--text-muted)]">No zone rules configured</p>
          <p className="text-sm mt-1">Create rules to control which asset domains can enter each gateway zone.</p>
          <button onClick={openModal} className="mt-4 bg-yellow-400 hover:bg-yellow-300 text-white font-semibold px-4 py-2 rounded-lg text-sm">
            ⊕ Create First Rule
          </button>
        </div>
      ) : (
        <div className="space-y-4">
          {gateways.map(gw => {
            ...
            return (
              <div key={gw.id} className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-xl overflow-hidden">
                <div className="px-5 py-3 bg-[var(--bg-surface)]/50 border-b border-[var(--border-subtle)] flex items-center gap-3">
                  <span className="text-yellow-400 font-mono text-xs">{gw.gateway_code}</span>
                  <span className="text-[var(--text-main)] font-semibold text-sm">{gw.name}</span>
                  {gw.location_name && <span className="text-[var(--text-faint)] text-xs">— {gw.location_name}</span>}
                  {gw.location_zone && (
                    <span className="ml-auto text-xs bg-[var(--bg-surface-hover)] text-[var(--text-muted)] px-2 py-0.5 rounded">{gw.location_zone}</span>
                  )}
                  <span className="text-xs text-[var(--text-faint)] ml-auto">{gwRules.length} rule{gwRules.length !== 1 ? 's' : ''}</span>
                </div>
                <div className="divide-y divide-white/5">
                  {gwRules.map(rule => (
                    <div key={rule.id} className="px-5 py-4 flex items-start gap-4 hover:bg-[var(--bg-surface-hover)] transition-colors">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-2">
                          <span className="text-[var(--text-main)] font-medium text-sm">{rule.rule_name}</span>
                          ...
                          {rule.status && (
                            <span className={`px-1.5 py-0.5 rounded text-xs border ${rule.status === 'ACTIVE' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-[var(--bg-surface-hover)] text-[var(--text-muted)] border-[var(--border-strong)]'}`}>
                              {rule.status}
                            </span>
                          )}
                        </div>
                        <div className="flex flex-wrap gap-1.5 mb-2">
                          <span className="text-xs text-[var(--text-faint)] mr-1 self-center">Allowed:</span>
                        ...
                        {rule.notes && <p className="text-xs text-[var(--text-faint)] italic">{rule.notes}</p>}
                      </div>
                      <div className="flex items-center gap-2 flex-shrink-0">
                        {rule.created_at && (
                          <span className="text-xs text-[var(--text-faint)]">{new Date(rule.created_at).toLocaleDateString()}</span>
                        )}
                        {deleteId === rule.id ? (
                          <div className="flex items-center gap-2">
                            ...
                            <button onClick={() => setDeleteId(null)} className="text-xs text-[var(--text-muted)] hover:text-[var(--text-main)]">No</button>
                          </div>
                        ) : (
                          <button
                            onClick={() => setDeleteId(rule.id)}
                            className="text-xs bg-[var(--bg-surface-hover)] hover:bg-red-900/50 text-[var(--text-muted)] hover:text-red-400 px-2 py-1 rounded transition-colors"
                          >
                            Delete
                          </button>
                        )}
          ...
          {unmappedRules.length > 0 && (
            <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-xl overflow-hidden">
              <div className="px-5 py-3 bg-[var(--bg-surface)]/50 border-b border-[var(--border-subtle)]">
                <span className="text-[var(--text-muted)] text-sm">Other Rules</span>
              </div>
              <div className="divide-y divide-white/5">
                {unmappedRules.map(rule => (
                  <div key={rule.id} className="px-5 py-4 flex items-start gap-4 hover:bg-[var(--bg-surface-hover)] transition-colors">
                    ...
                    <button onClick={() => setDeleteId(rule.id)} className="text-xs bg-[var(--bg-surface-hover)] hover:bg-red-900/50 text-[var(--text-muted)] hover:text-red-400 px-2 py-1 rounded">Delete</button>
                  </div>
                ))}
              </div>
            </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={() => setShowModal(false)}>
          <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-5 border-b border-[var(--border-subtle)]">
              <h2 className="text-[var(--text-main)] font-semibold">⊕ New Zone Rule</h2>
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                {selectedGateway?.location_zone && (
                  <p className="text-xs text-[var(--text-faint)] mt-1">Zone: {selectedGateway.location_zone}</p>
                )}
              </div>
              ...
              {/* Allowed domains */}
              <div>
                <label className="block text-xs text-[var(--text-muted)] mb-2">Allowed Domains* <span className="text-[var(--text-faint)]">(select all that apply)</span></label>
                <div className="grid grid-cols-2 gap-2">
                  {DOMAINS.map(domain => {
                    ...
                    return (
                      ...
                      >
                        <span className={`w-4 h-4 rounded flex-shrink-0 border flex items-center justify-center text-xs ${selected ? 'bg-current border-current' : 'border-[var(--border-strong)]'}`}>
                          {selected && <span className="text-[var(--text-main)] font-bold">✓</span>}
                        </span>
                        {domain}
                      </button>
                    );
              ...
              {/* Severity */}
              <div>
                <label className="block text-xs text-[var(--text-muted)] mb-1">Violation Severity</label>
                <select
                  value={form.severity}
              ...
              {/* Alert toggle */}
              <div className="flex items-center justify-between bg-[var(--bg-surface)] rounded-lg px-4 py-3 border border-[var(--border-subtle)]">
                <div>
                  <p className="text-sm text-[var(--text-main)] font-medium">Alert on Violation</p>
                  <p className="text-xs text-[var(--text-muted)]">Generate an alert when unauthorized domain enters this zone</p>
                </div>
                <button
                  type="button"
                  onClick={() => setForm(f => ({ ...f, alert_on_violation: !f.alert_on_violation }))}
                  className={`w-11 h-6 rounded-full transition-all relative flex-shrink-0 ${form.alert_on_violation ? 'bg-yellow-400' : 'bg-[var(--bg-surface-hover)]'}`}
                >
            ...
            <div className="flex gap-3 justify-end p-5 border-t border-[var(--border-subtle)]">
              ...
              <button
                ...
                className="bg-yellow-400 hover:bg-yellow-300 text-white font-semibold px-5 py-2 rounded-lg text-sm disabled:opacity-50"
              >
