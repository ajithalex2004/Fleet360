  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-[var(--text-muted)]">Pin your driver code so the Trips tab filters to your assigned routes.</p>
      </div>
      <div className="rounded-2xl bg-[var(--bg-surface)]/60 border border-[var(--border-subtle)] p-5 space-y-4">
        <div>
          <label className="block text-xs uppercase tracking-wide text-[var(--text-muted)] mb-1.5 font-semibold">Driver Code / ID *</label>
          <input value={code} onChange={e => setCode(e.target.value)} placeholder="e.g. d-1234"
            className="w-full px-4 py-3 rounded-xl bg-[var(--bg-surface)]/60 border border-[var(--border-subtle)] text-[var(--text-main)] text-lg font-mono focus:border-rose-500 focus:outline-none" />
        </div>
        <div>
          <label className="block text-xs uppercase tracking-wide text-[var(--text-muted)] mb-1.5 font-semibold">Display Name</label>
