  ...
  const handleSave = async (route: RouteResult, waypoints: Waypoint[]) => {
    ...
    try {
      ...
      const res = await fetch('/api/logistics/shipments', {
        ...
        body: JSON.stringify({
          cargoOwnerName: 'Route Planner',
          originName: origin?.label ?? '',
          destinationName: destination?.label ?? '',
          shipmentType: 'FTL',
          pickupWindowFrom: new Date().toISOString(),
          status: 'PENDING',
      ...
      setSaved(data.data?.shipmentNo ?? data.shipmentNo ?? 'Shipment order');
    ...
    <div className="space-y-6">
      ...
      {saved && (
        <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl px-4 py-3 flex items-center gap-3">
          ...
          <div>
            <p className="text-emerald-300 text-sm font-semibold">Route saved as shipment order <span className="font-mono">{saved}</span></p>
