...
export default function ShipmentManifestPage() {
  ...
  if (loading) return <div className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-surface)]/70 p-8 text-[var(--text-muted)]">Loading manifest...</div>;
  if (!data) return <div className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-surface)]/70 p-8 text-[var(--text-muted)]">Shipment manifest not found.</div>;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 print:hidden">
        <div>
          <div className="mb-1 flex items-center gap-2 text-xs text-[var(--text-faint)]">
            ...
            <span>/</span>
            <span className="font-mono text-[var(--text-muted)]">{data.shipment.shipmentNo ?? id?.slice(0, 8)}</span>
          </div>
          <h1 className="flex items-center gap-2 text-2xl font-bold text-[var(--text-main)]"><ClipboardList className="h-6 w-6 text-amber-300" /> Cargo Manifest</h1>
          <p className="mt-1 text-xs text-[var(--text-muted)]">{data.summary.totalStops} stops - {data.summary.delivered} delivered - {data.summary.pending} pending</p>
        </div>
        <div className="flex gap-2">
          <button type="button" onClick={() => window.print()} className="inline-flex items-center gap-1.5 rounded-xl border border-[var(--border-subtle)] px-3 py-2 text-sm text-[var(--text-muted)] hover:bg-[var(--bg-surface-hover)]"><Printer className="h-4 w-4" /> Print</button>
          <button type="button" onClick={() => setShowAdd(true)} className="inline-flex items-center gap-1.5 rounded-xl bg-amber-500 px-4 py-2 text-sm font-semibold text-white hover:bg-amber-400"><Plus className="h-4 w-4" /> Add stop</button>
        </div>
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        {data.stops.map(stop => (
          <article key={stop.id} className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-surface)]/70 p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-xs uppercase tracking-wide text-[var(--text-faint)]">Stop {stop.stop_number}</p>
                <h2 className="mt-1 text-lg font-semibold text-[var(--text-main)]">{stop.stop_name ?? 'Stop'}</h2>
                <p className="text-sm text-[var(--text-muted)]">{stop.stop_address ?? '-'}</p>
              </div>
              <span className={`rounded-full border px-2 py-1 text-xs ${stop.status === 'DELIVERED' ? 'border-emerald-500/30 text-emerald-300' : stop.status === 'SKIPPED' ? 'border-slate-500/30 text-[var(--text-muted)]' : 'border-amber-500/30 text-amber-300'}`}>{stop.status}</span>
            </div>
            <div className="mt-3 grid gap-2 text-sm text-[var(--text-muted)]">
              <p>Recipient: <span className="text-[var(--text-main)]">{stop.recipient ?? '-'}</span></p>
              {stop.delivery_note && <p>Note: <span className="text-[var(--text-main)]">{stop.delivery_note}</span></p>}
...
function ConfirmModal({ shipmentId, stop, onClose, onSaved }: { shipmentId: string; stop: ManifestStop; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  return <Modal title={`Confirm stop ${stop.stop_number}`} onClose={onClose}><label className="block"><span className="mb-1 block text-[11px] uppercase tracking-wide text-[var(--text-faint)]">Delivery note</span><textarea value={note} onChange={e => setNote(e.target.value)} className="min-h-24 w-full rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] px-3 py-2 text-sm text-[var(--text-main)]" /></label><div className="mt-5 flex justify-end gap-2"><button type="button" onClick={onClose} className="rounded-xl border border-[var(--border-subtle)] px-4 py-2 text-sm text-[var(--text-muted)]">Cancel</button><button type="button" onClick={() => void save()} className="rounded-xl bg-emerald-500 px-4 py-2 text-sm font-semibold text-white">Confirm delivered</button></div></Modal>;
}

function Modal({ title, onClose, children }: { title: string; onClose: () => void; children: React.ReactNode }) {
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4"><div className="w-full max-w-xl rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-canvas)] p-5 shadow-2xl"><div className="mb-4 flex items-center justify-between"><h2 className="text-lg font-bold text-[var(--text-main)]">{title}</h2><button type="button" onClick={onClose} className="rounded-lg p-1 text-[var(--text-muted)] hover:bg-[var(--bg-surface-hover)] hover:text-[var(--text-main)]"><X className="h-5 w-5" /></button></div>{children}</div></div>;
}
...
function Info({ label, value }: { label: string; value: string }) {
  return <div><p className="text-xs text-[var(--text-faint)]">{label}</p><p className="text-[var(--text-main)]">{value}</p></div>;
}
