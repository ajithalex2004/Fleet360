...
type TransferReason = 'REBALANCING' | 'CUSTOMER_DROPOFF' | 'MAINTENANCE' | 'OVERFLOW' | 'OTHER';
...
const REASONS: { value: TransferReason; label: string }[] = [
  ...
  { value: 'CUSTOMER_DROPOFF', label: 'Customer Drop-off' },
  { value: 'MAINTENANCE',    label: 'Maintenance' },
  { value: 'OVERFLOW',       label: 'Fleet Overflow' },
...
export default function RentalTransfersPage() {
  ...
  const fetchTransfers = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/rental/transfers?${params}`);
  ...
  const handleCreate = async () => {
    ...
    try {
      const res = await fetch('/api/rental/transfers', {
        method: 'POST',
  ...
  const handleAction = async (id: string, action: string, extra?: Record<string, string>) => {
    setSaving(true); setError('');
    try {
      const res = await fetch(`/api/rental/transfers?id=${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const pipelineSteps: { key: TransferStatus; label: string; count: number; color: string }[] = [
    ...
    { key: 'APPROVED',   label: 'Approved',   count: summary.approved,   color: 'border-teal-500/50    bg-teal-500/10    text-teal-300'    },
    { key: 'IN_TRANSIT', label: 'In Transit', count: summary.inTransit,  color: 'border-cyan-500/50    bg-cyan-500/10    text-cyan-300'    },
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-teal-600 to-cyan-600 text-white text-sm font-medium hover:opacity-90 transition-opacity"
        >
          ...
          <div className="bg-slate-900 border border-white/10 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="flex gap-3 p-6 border-t border-white/5">
              ...
              <button onClick={handleCreate} disabled={saving}
                className="flex-1 px-4 py-2 rounded-lg bg-gradient-to-r from-teal-600 to-cyan-600 text-white text-sm font-medium hover:opacity-90 transition-opacity disabled:opacity-50">
