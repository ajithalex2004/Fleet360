...
function TransitionModal({ agreement, onClose, onDone }: {
  ...
  const actions: Array<{ key: typeof action; label: string; show: boolean; color: string }> = [
    { key: 'activate'      as const, label: '🟢 Activate Agreement',  show: status === 'DRAFT',     color: 'bg-emerald-600 hover:bg-emerald-500' },
    { key: 'sign'          as const, label: '✍️ Mark as Signed',       show: status === 'DRAFT',     color: 'bg-blue-600 hover:bg-blue-500' },
    { key: 'complete'      as const, label: '✅ Complete & Return',    show: status === 'ACTIVE',    color: 'bg-teal-600 hover:bg-teal-500' },
    { key: 'refund_deposit' as const, label: '💰 Refund Security Dep.', show: agreement.depositStatus === 'PAID', color: 'bg-violet-600 hover:bg-violet-500' },
    { key: 'cancel'        as const, label: '❌ Cancel Agreement',     show: ['DRAFT','ACTIVE'].includes(status), color: 'bg-red-700 hover:bg-red-600' },
  ].filter(a => a.show);
