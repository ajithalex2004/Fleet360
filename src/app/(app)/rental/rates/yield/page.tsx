...
const LAYER_COLORS: Record<string, { bg: string; border: string; text: string }> = {
  BASE:        { bg: 'bg-[var(--bg-surface-hover)]/40',   border: 'border-[var(--border-strong)]',     text: 'text-[var(--text-main)]' },
...
export default function YieldAnalyzerPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      ...
      {/* Inputs */}
      <div className="bg-[var(--bg-surface)]/50 border border-teal-500/20 rounded-2xl p-6">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          <div>
            <label className="text-xs text-[var(--text-muted)] block mb-1">Vehicle category</label>
            <select
              value={category}
              onChange={(e) => setCategory(e.target.value)}
              className="w-full px-3 py-2 rounded-lg bg-[var(--bg-surface)]/60 border border-[var(--border-subtle)] text-[var(--text-main)] text-sm"
            >
              {CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
            </select>
          </div>
          <div>
            <label className="text-xs text-[var(--text-muted)] block mb-1">Pickup date</label>
            <input
              type="date"
              value={pickupDate}
          ...
          <div>
            <label className="text-xs text-[var(--text-muted)] block mb-1">Drop-off date</label>
            <input
              type="date"
              value={dropoffDate}
      ...
      {result && diag && (
        <div className="space-y-3">
          {/* Summary */}
          <div className="bg-[var(--bg-surface)]/50 border border-emerald-500/30 rounded-2xl p-6">
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <div>
                <div className="text-xs text-[var(--text-muted)] uppercase tracking-wider">Final daily rate</div>
                <div className="text-3xl font-bold text-[var(--text-main)] mt-1">
              ...
              <div className="text-right">
                <div className="text-xs text-[var(--text-muted)] uppercase tracking-wider">Total for {result.totalDays} days</div>
            ...
            <div className="mt-3 flex gap-3 text-xs text-[var(--text-muted)] flex-wrap">
              <span>Rule: <strong className="text-[var(--text-main)]">{result.ruleName}</strong></span>
              <span>·</span>
              <span>Channel: <strong className="text-[var(--text-main)]">{result.channel}</strong></span>
              <span>·</span>
              ...
              {diag.utilizationPctUsed != null && (
                <>
                  <span>·</span>
                  <span>Utilization: <strong className="text-[var(--text-main)]">{diag.utilizationPctUsed}%</strong>{diag.utilizationAuto ? ' (auto)' : ' (override)'}</span>
                </>
              )}
            </div>
          </div>
                ...
                <React.Fragment key={i}>
                  {i > 0 && (
                    <div className="flex justify-center">
                      <ArrowDown className="h-4 w-4 text-[var(--text-faint)]" />
                    </div>
                  )}
                  <div className={`rounded-xl border ${c.bg} ${c.border} p-4`}>
                    <div className="flex items-center justify-between gap-3 flex-wrap">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-semibold text-[var(--text-main)]">{layer.label}</span>
                      </div>
                      <div className="text-right">
                    ...
                    <p className="text-xs text-[var(--text-muted)] mt-2">{layer.rationale}</p>
                  </div>
                </React.Fragment>
              );
            })}
