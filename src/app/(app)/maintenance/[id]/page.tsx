        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="mt-1 text-[var(--text-faint)]">Created on {new Date(request.requestDate).toLocaleDateString()}</p>
                </div>
                <button
                    onClick={() => router.back()}
                    className="text-sm font-medium text-[var(--text-faint)] hover:text-[var(--text-main)]"
                >
                    ← Back to Requests
                </button>
            </div>
            ...
            <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] p-6 shadow-sm">
                <h2 className="mb-4 text-lg font-semibold text-[var(--text-main)]">Workflow Progress</h2>
            ...
            <div className="grid gap-6 md:grid-cols-3">
                <div className="md:col-span-2 space-y-6">
                    <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] p-6 shadow-sm">
                        <h2 className="mb-4 text-lg font-semibold text-[var(--text-main)]">Issue Details</h2>

                        {/* Request Meta Info */}
                        <div className="mb-6 grid grid-cols-2 gap-4 rounded-lg bg-[var(--bg-surface)]/50 p-4">
                            <div>
                                <p className="text-sm font-medium text-[var(--text-faint)]">Requested By</p>
                                <p className="font-medium text-[var(--text-main)]">{driver ? driver.name : 'Unknown'}</p>
                                {driver && <p className="text-xs text-[var(--text-faint)]">{driver.contactNumber}</p>}
                            </div>
                            <div>
                                <p className="text-sm font-medium text-[var(--text-faint)]">Requested On</p>
                        ...
                        <p className="text-[var(--text-muted)] mb-6">
                            <span className="block text-sm font-medium text-[var(--text-faint)] mb-1">Description</span>
                            {request.description}
                        </p>

                        {request.estimatedCost && (
                            <div className="mt-6 rounded-lg bg-[var(--bg-surface)]/50 p-4">
                                <p className="text-sm font-medium text-[var(--text-faint)]">Estimated Cost</p>
                                <p className="text-2xl font-bold text-[var(--text-main)]">${request.estimatedCost}</p>
                            </div>
                        )}
                        ...
                        {currentGarage && (
                            <div className="mt-6 rounded-lg bg-[var(--bg-surface)]/50 p-4">
                                <p className="text-sm font-medium text-[var(--text-faint)]">Assigned Garage</p>
                                <p className="text-lg font-bold text-[var(--text-main)]">{currentGarage.name}</p>
                                <p className="text-sm text-[var(--text-faint)]">{currentGarage.location}</p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] p-6 shadow-sm">
                        <h2 className="mb-4 text-lg font-semibold text-[var(--text-main)]">Actions</h2>
                        <div className="flex flex-col gap-4">
                            {request.status === MaintenanceStatus.UNDER_ESTIMATION && (
                                <div className="w-full">
                                    <label className="block text-sm font-medium text-[var(--text-muted)] mb-2">Select Garages for Estimation</label>
                                    ...
                                    <div className="mb-3">
                                        <div className="relative">
                                            <input
                                                ...
                                                className="w-full rounded-lg border border-[var(--border-subtle)] px-3 py-2 pl-10 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 text-[var(--text-main)]"
                                            />
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="absolute left-3 top-2.5 h-5 w-5 text-[var(--text-muted)]">
                                    ...
                                    {/* Garage List */}
                                    <div className="max-h-64 overflow-y-auto rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-surface)]/50 p-2">
                                        {garages
                                            .filter(g =>
                                            ...
                                            .map(g => (
                                                <label
                                                    ...
                                                    className={`flex items-center space-x-3 p-3 rounded-lg cursor-pointer transition-all border mb-2 ${selectedGarages.includes(g.id)
                                                        ...
                                                        : 'bg-[var(--bg-surface)] border-transparent hover:bg-[var(--bg-surface)]/5 hover:border-[var(--border-subtle)]'
                                                        }`}
                                                >
                                                    <input
                                                        type="checkbox"
                                                    ...
                                                    <div className="flex-1">
                                                        <div className="flex items-center justify-between">
                                                            <span className="font-medium text-[var(--text-main)]">{g.name}</span>
                                                            {g.isInternal && (
                                                                <span className="text-xs bg-emerald-500/20 text-green-700 px-2 py-0.5 rounded-full">Internal</span>
                                                            )}
                                                        </div>
                                                        <p className="text-xs text-[var(--text-faint)] mt-0.5">{g.location}</p>
                                                        <div className="flex flex-wrap gap-1 mt-1">
                                                            {g.specialties.slice(0, 3).map(specialty => (
                                                                <span key={specialty} className="text-xs bg-[var(--bg-surface-hover)]/40 text-[var(--text-faint)] px-1.5 py-0.5 rounded">
                                                                    {specialty}
                                                                </span>
                                                            ))}
                                                            {g.specialties.length > 3 && (
                                                                <span className="text-xs text-[var(--text-muted)]">+{g.specialties.length - 3} more</span>
                                                            )}
                                                        </div>
                                                    </div>
                                                </label>
                            ...
                            <div className="flex gap-3 flex-wrap">
                                ...
                                {request.status === MaintenanceStatus.INVOICE_SUBMITTED && (
                                    <button
                                        ...
                                        className="rounded-lg bg-[var(--bg-surface-hover)] px-4 py-2 text-sm font-medium text-[var(--text-main)] hover:bg-[var(--bg-surface-hover)]"
                                    >
                                        Close Request
                                    </button>
                                )}
                                {request.status === MaintenanceStatus.CLOSED && (
                                    <p className="text-sm text-[var(--text-faint)] font-medium">Request Closed</p>
                                )}
                            </div>
                        </div>
                    </div>
                ...
                <div className="space-y-6">
                    <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] p-6 shadow-sm">
                        <h2 className="mb-4 text-lg font-semibold text-[var(--text-main)]">Vehicle Info</h2>
                        {vehicle ? (
                            <div className="space-y-3">
                                <div>
                                    <p className="text-sm text-[var(--text-faint)]">Vehicle</p>
                                    <p className="font-medium text-[var(--text-main)]">{vehicle.make} {vehicle.model}</p>
                                </div>
                                <div>
                                    <p className="text-sm text-[var(--text-faint)]">License Plate</p>
                                    <p className="font-medium text-[var(--text-main)]">{vehicle.licensePlate}</p>
                                </div>
                                <div>
                                    <p className="text-sm text-[var(--text-faint)]">Mileage</p>
                                    <p className="font-medium text-[var(--text-main)]">{vehicle.currentMileage.toLocaleString()} km</p>
                                </div>
                                <div>
                                    <p className="text-sm text-[var(--text-faint)]">VIN</p>
                                    <p className="font-medium text-[var(--text-main)]">{vehicle.vin}</p>
                                </div>
                            </div>
                        ) : (
                            <p className="text-[var(--text-faint)]">Loading vehicle info...</p>
                        )}
                    </div>
                    ...
                    <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface)] p-6 shadow-sm">
                        <div className="flex items-center justify-between mb-4">
                            <h2 className="text-lg font-semibold text-[var(--text-main)]">Attachments</h2>
                            <button
                        ...
                        {request.attachments && request.attachments.length > 0 ? (
                            <div className="space-y-3">
                                {request.attachments.map((att) => (
                                    <div key={att.id} className="flex items-center justify-between rounded-lg border border-[var(--border-subtle)] bg-[var(--bg-surface)]/50 p-3">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-[var(--text-faint)]">{att.type} • {new Date(att.uploadedAt).toLocaleDateString()}</p>
                                            </div>
                                        </div>
                            ...
                            <div className="rounded-lg border border-dashed border-[var(--border-subtle)] p-8 text-center text-[var(--text-faint)]">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="mx-auto h-12 w-12 text-[var(--text-muted)] mb-2">
                                    <path strokeLinecap="round" strokeLinejoin="round" d="m18.375 12.739-7.693 7.693a4.5 4.5 0 0 1-6.364-6.364l10.94-10.94A3 3 0 1 1 19.5 7.372L8.552 18.32m.009-.01-.01.01m5.699-9.941-7.81 7.81a1.5 1.5 0 0 0 2.112 2.13" />
                                </svg>
                ...
                showAttachmentModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                        <div className="w-full max-w-md rounded-xl bg-[var(--bg-surface)] p-6 shadow-2xl">
                            <h3 className="text-lg font-bold text-[var(--text-main)]">Add Attachment</h3>
                            <p className="mt-1 text-sm text-[var(--text-faint)]">Select the attachment type and upload a file</p>
                            ...
                            <div className="mt-6 flex justify-end gap-3">
                                <button
                                    ...
                                    className="rounded-lg px-4 py-2 text-sm font-medium text-[var(--text-muted)] hover:bg-[var(--bg-surface)]/10"
                                >
                                    Cancel
                                </button>
                                <button
