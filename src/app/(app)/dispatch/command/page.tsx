...
// SCHOOL_BUS and AMBULANCE services exist in the DB but are deliberately excluded from
// Dispatch Command Centre. Each has a purpose-built dispatch board in its own module:
...
type SvcType     = 'PASSENGER'|'FREIGHT'|'DELIVERY'|'TECHNICIAN'|'SCHOOL_BUS'|'AMBULANCE';
...
interface Job {
  ...
  scheduled_pickup?: string; passenger_count?: number;
    ...
    try {
      ...
      const pairs: MergePair[] = d.suggestions.map((s: Record<string,unknown>) => {
        ...
        const candidateJob: Job = {
          ...
          updated_at:          String(s.job_b_updated_at ?? new Date().toISOString()),
