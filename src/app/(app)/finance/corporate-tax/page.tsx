    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-2xl w-full max-w-md">
        <div className="flex items-center justify-between p-5 border-b border-[var(--border-subtle)]">
          <h2 className="text-lg font-bold text-[var(--text-main)]">Create CT Return</h2>
          <button onClick={onClose} className="text-[var(--text-muted)] hover:text-[var(--text-main)] text-xl">×</button>
        </div>
        <div className="p-5 space-y-3">
          <div>
            <label className="block text-xs text-[var(--text-muted)] mb-1">Tax Year</label>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[var(--text-main)]">UAE Corporate Tax</h1>
          <p className="text-[var(--text-muted)] text-xs mt-0.5">9% CT on taxable income above AED 375,000 · FTA Compliance</p>
        </div>
      ...
      <div className="bg-gradient-to-r from-slate-800/60 to-emerald-900/20 border border-emerald-500/20 rounded-2xl p-4">
        <div className="grid grid-cols-4 gap-4 text-center">
          <div>
            <p className="text-xs text-[var(--text-muted)]">CT Rate</p>
            <p className="text-2xl font-bold text-emerald-400 mt-1">9%</p>
            <p className="text-xs text-[var(--text-faint)]">on income above threshold</p>
          </div>
          <div>
            <p className="text-xs text-[var(--text-muted)]">Threshold (0% band)</p>
            <p className="text-2xl font-bold text-[var(--text-main)] mt-1">AED 375K</p>
            <p className="text-xs text-[var(--text-faint)]">zero-rate below this</p>
          </div>
          <div>
            <p className="text-xs text-[var(--text-muted)]">Small Business Relief</p>
            <p className="text-2xl font-bold text-[var(--text-main)] mt-1">AED 3M</p>
            <p className="text-xs text-[var(--text-faint)]">revenue threshold</p>
          </div>
          <div>
            <p className="text-xs text-[var(--text-muted)]">Filing Deadline</p>
            <p className="text-2xl font-bold text-amber-400 mt-1">9 months</p>
            <p className="text-xs text-[var(--text-faint)]">after fiscal year end</p>
          </div>
        </div>
      </div>

      {/* Live Estimate */}
      {loading ? <div className="h-48 bg-[var(--bg-surface)]/60 rounded-2xl animate-pulse" /> : estimate && (
        <div className="bg-[var(--bg-surface)]/60 border border-[var(--border-subtle)] rounded-2xl overflow-hidden">
          <div className="px-5 py-3.5 border-b border-[var(--border-subtle)] flex items-center justify-between">
            <h2 className="text-sm font-bold text-[var(--text-main)]">FY {year} — Live CT Estimate</h2>
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="space-y-0">
                {[
                  ...
                  { label: 'TAXABLE INCOME',              value: estimate.taxableIncome,     color: 'text-[var(--text-main)]',       border: false, bold: true },
                  { label: 'Less: Threshold (0% band)',   value: -estimate.threshold,        color: 'text-[var(--text-muted)]',   border: false },
                  { label: 'Income Above Threshold',      value: estimate.aboveThreshold,    color: 'text-[var(--text-muted)]',   border: true  },
                  ...
                  <div key={i} className={`flex items-center justify-between py-2 ${row.border ? 'border-b border-[var(--border-strong)] mb-1' : ''}`}>
                    <span className={`text-sm ${row.bold ? 'font-bold text-[var(--text-main)]' : 'text-[var(--text-muted)]'}`}>{row.label}</span>
              ...
              <div className="space-y-3">
                <div className={`p-4 rounded-2xl border ${estimate.ctLiability === 0 ? 'bg-emerald-900/20 border-emerald-500/30' : 'bg-red-900/20 border-red-500/30'}`}>
                  <p className="text-xs text-[var(--text-muted)]">Estimated CT Liability</p>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-[var(--bg-surface)]/60 rounded-xl p-3">
                    ...
                    <p className="text-xl font-bold text-[var(--text-main)] mt-1">{estimate.effectiveRate}%</p>
                  </div>
                  <div className="bg-[var(--bg-surface)]/60 rounded-xl p-3">
                    <p className="text-xs text-[var(--text-muted)]">Filing Deadline</p>
                ...
                <div className="bg-[var(--bg-surface)]/60 rounded-xl p-3">
                  <p className="text-xs text-[var(--text-muted)]">Taxable vs Total Revenue</p>
                  <div className="mt-2 bg-[var(--bg-surface-hover)] rounded-full h-2">
      ...
      {/* CT Returns History */}
      <div>
        <h2 className="text-sm font-bold text-[var(--text-main)] mb-3">CT Returns History</h2>
        {returns.length === 0 ? (
          <div className="bg-[var(--bg-surface)]/60 border border-[var(--border-subtle)] rounded-2xl p-8 text-center">
            <p className="text-[var(--text-faint)] text-sm">No CT returns created yet.</p>
          </div>
        ) : (
          <div className="bg-[var(--bg-surface)]/60 border border-[var(--border-subtle)] rounded-2xl overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                {returns.map(r => (
                  <tr key={r.id} className="border-b border-[var(--border-subtle)] hover:bg-[var(--bg-surface)]/40">
                    <td className="px-4 py-3 font-bold text-[var(--text-main)]">FY {r.tax_year}</td>
                    <td className="px-4 py-3 text-xs text-[var(--text-muted)]">{r.period_from} → {r.period_to}</td>
                    ...
                    <td className="px-4 py-3 text-right text-sm text-[var(--text-muted)]">AED {fmt(parseFloat(r.revenue))}</td>
                    <td className="px-4 py-3 text-right text-sm text-[var(--text-muted)]">AED {fmt(parseFloat(r.taxable_income))}</td>
                    ...
                    <td className={`px-4 py-3 text-xs ${new Date(r.filing_deadline) < new Date() && r.status !== 'FILED' ? 'text-red-400 font-bold' : 'text-[var(--text-muted)]'}`}>
                      {r.filing_deadline}
                    </td>
                    <td className="px-4 py-3">
                      {r.status === 'DRAFT' && (
