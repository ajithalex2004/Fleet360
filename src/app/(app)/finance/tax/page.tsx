...
const CAT_STYLE: Record<string, string> = {
  ...
  EXEMPT:       'bg-slate-500/20 text-[var(--text-muted)] border-slate-500/30',
...
function CategoryModal({ cat, onClose, onSaved }: { cat?: TaxCategory; onClose: () => void; onSaved: () => void }) {
  ...
  const inp = 'w-full bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-lg px-3 py-2 text-sm text-[var(--text-main)] focus:outline-none focus:border-amber-500';
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-2xl w-full max-w-md">
        <div className="flex items-center justify-between p-5 border-b border-[var(--border-subtle)]">
          <h2 className="text-lg font-bold text-[var(--text-main)]">{cat ? 'Edit' : 'Add'} Tax Category</h2>
          <button onClick={onClose} className="text-[var(--text-muted)] hover:text-[var(--text-main)] text-xl">×</button>
        </div>
        <div className="p-5 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-[var(--text-muted)] mb-1">Code</label>
              <input value={form.code} onChange={s('code')} placeholder="STANDARD" className={inp} />
            </div>
            <div>
              <label className="block text-xs text-[var(--text-muted)] mb-1">Rate (%)</label>
          ...
          <div>
            ...
            <textarea value={form.description} onChange={s('description')} rows={2}
              className="w-full bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-lg px-3 py-2 text-sm text-[var(--text-main)] resize-none focus:outline-none focus:border-amber-500" />
          </div>
          <div>
            <label className="block text-xs text-[var(--text-muted)] mb-1">FTA Box Code</label>
          ...
          <div className="flex items-center gap-4">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-sm text-[var(--text-muted)]">Default Rate</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[var(--text-main)]">Tax Engine</h1>
          <p className="text-[var(--text-muted)] text-xs mt-0.5">UAE VAT — FTA compliance with Input/Output tracking</p>
        </div>
      ...
      {/* Tabs */}
      <div className="flex bg-[var(--bg-surface)]/60 border border-[var(--border-subtle)] rounded-xl p-1 gap-1 w-fit">
        {(['overview', 'categories', 'audit'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 py-2 rounded-lg text-sm font-medium capitalize transition-all ${tab === t ? 'bg-amber-600 text-white' : 'text-[var(--text-muted)] hover:text-[var(--text-main)]'}`}>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-[var(--bg-surface)]/60 border border-[var(--border-subtle)] rounded-2xl p-5">
                  <p className="text-xs text-[var(--text-faint)]">📤 Output VAT (Collected)</p>
                  <p className="text-3xl font-bold text-amber-400 mt-2">{fmtAED(summary.output.vat)}</p>
                  <p className="text-xs text-[var(--text-faint)] mt-1">From {summary.output.invoices} invoices</p>
                </div>
                <div className="bg-[var(--bg-surface)]/60 border border-[var(--border-subtle)] rounded-2xl p-5">
                  <p className="text-xs text-[var(--text-faint)]">📥 Input VAT (Recoverable)</p>
                  <p className="text-3xl font-bold text-blue-400 mt-2">{fmtAED(summary.input.vat)}</p>
                  <p className="text-xs text-[var(--text-faint)] mt-1">Fuel + maintenance purchases</p>
                </div>
                <div className={`border rounded-2xl p-5 ${summary.net.payable ? 'bg-red-900/20 border-red-500/30' : 'bg-emerald-900/20 border-emerald-500/30'}`}>
                  <p className="text-xs text-[var(--text-faint)]">{summary.net.payable ? '⚠️ Net VAT Payable' : '✅ VAT Refundable'}</p>
                  <p className={`text-3xl font-bold mt-2 ${summary.net.payable ? 'text-red-400' : 'text-emerald-400'}`}>
                    {fmtAED(Math.abs(summary.net.vat))}
                  </p>
                  <p className="text-xs text-[var(--text-faint)] mt-1">Q{summary.period.quarter} {summary.period.year}</p>
                </div>
              </div>
              ...
              <div className="bg-[var(--bg-surface)]/60 border border-[var(--border-subtle)] rounded-2xl p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-[var(--text-main)]">Q{summary.period.quarter} {summary.period.year} VAT Period</p>
                    <p className="text-xs text-[var(--text-muted)]">{summary.period.startDate} → {summary.period.endDate}</p>
                  </div>
              ...
              <div className="bg-[var(--bg-surface)]/60 border border-[var(--border-subtle)] rounded-2xl p-5">
                <h3 className="text-sm font-bold text-[var(--text-main)] mb-4">UAE FTA VAT Return — Box Mapping</h3>
                <div className="space-y-2">
                  {[
                    ...
                    <div key={row.box} className="grid grid-cols-4 gap-3 text-xs py-2 border-b border-[var(--border-subtle)] last:border-0">
                      <span className="text-amber-400 font-mono font-bold">Box {row.box}</span>
                      <span className="col-span-2 text-[var(--text-muted)]">{row.label}</span>
                      <span className={`text-right font-semibold ${row.box === '12' ? (summary.net.payable ? 'text-red-400' : 'text-emerald-400') : 'text-[var(--text-main)]'}`}>
                        {fmtAED(row.vat)}
                      </span>
                    </div>
                  ))}
      ...
      {tab === 'categories' && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            {categories.map(cat => (
              <div key={cat.id} className="bg-[var(--bg-surface)]/60 border border-[var(--border-subtle)] rounded-2xl p-5">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium border ${CAT_STYLE[cat.code] ?? 'text-[var(--text-muted)]'}`}>
                      {cat.code}
                    </span>
                    {cat.is_default && <span className="ml-2 text-xs text-amber-400">★ Default</span>}
                  </div>
                  <button onClick={() => { setEditCat(cat); setShowCatModal(true); }}
                    className="text-xs text-[var(--text-muted)] hover:text-[var(--text-main)] px-2 py-1 rounded-lg hover:bg-[var(--bg-surface-hover)]">
                    Edit
                  </button>
                </div>
                <p className="text-xl font-bold text-[var(--text-main)]">{cat.rate}%</p>
                <p className="text-sm font-medium text-[var(--text-main)] mt-1">{cat.name}</p>
    ...
    <div className="bg-[var(--bg-surface)]/60 border border-[var(--border-subtle)] rounded-2xl overflow-hidden">
      {logs.length === 0 ? (
        <div className="p-12 text-center">
          ...
          <p className="text-[var(--text-faint)] text-xs mt-1">Recalculate or submit a VAT return to create audit entries</p>
        </div>
      ) : (
        <table className="w-full text-sm">
          ...
          <tbody>
            {logs.map(l => (
              <tr key={String(l.id)} className="border-b border-[var(--border-subtle)] last:border-0 hover:bg-[var(--bg-surface)]/40">
                ...
                <td className="px-5 py-3 text-[var(--text-muted)] text-xs">{String(l.entity_type ?? '—')}</td>
                <td className="px-5 py-3 text-[var(--text-muted)] text-xs">{String(l.performed_by ?? '—')}</td>
                <td className="px-5 py-3 text-[var(--text-muted)] text-xs">{String(l.notes ?? '—')}</td>
                <td className="px-5 py-3 text-[var(--text-muted)] text-xs">{new Date(String(l.created_at)).toLocaleString('en-AE')}</td>
              </tr>
            ))}
          </tbody>
        </table>
