...
export default function BranchPLPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-[var(--text-main)]">Branch P&amp;L Report</h1>
        <p className="text-[var(--text-muted)] text-xs mt-1">Cost-center segmented income statement · Multi-emirate branch comparison</p>
      </div>
      ...
      <div className="flex flex-wrap gap-3 items-end">
        ...
        <div>
          <label className="block text-[var(--text-muted)] text-xs mb-1.5">From</label>
        ...
        <div>
          <label className="block text-[var(--text-muted)] text-xs mb-1.5">To</label>
      ...
      {/* Tenant header */}
      {data && (
        <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-2xl p-5 flex items-center gap-4">
          <span className="text-3xl">🏢</span>
          <div>
            <p className="text-[var(--text-main)] font-bold text-lg">{data.tenant.name}</p>
            <div className="flex items-center gap-4 mt-1">
              {data.tenant.trn && <span className="text-xs text-[var(--text-muted)] font-mono">TRN: <strong className="text-[var(--text-main)]">{data.tenant.trn}</strong></span>}
              <span className="text-xs text-[var(--text-faint)]">Period: {data.period.start} → {data.period.end}</span>
              <span className="text-xs text-[var(--text-faint)]">{branches.length} branch{branches.length !== 1 ? 'es' : ''}</span>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            <div key={k.label} className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-2xl p-5">
              ...
              {k.sub && <p className="text-[var(--text-faint)] text-xs mt-1">{k.sub}</p>}
            </div>
          ))}
        </div>
      )}
      ...
      {branches.length > 0 && (
        <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-2xl p-5">
          <h2 className="text-[var(--text-main)] font-semibold mb-5">Revenue by Branch</h2>
                ...
                <div key={b.branch_id ?? 'unassigned'}>
                  <div className="flex items-center justify-between text-xs mb-1.5">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[var(--text-muted)] font-medium">{b.branch_name}</span>
                      {b.cost_center_code && (
                        <span className="font-mono bg-[var(--bg-surface)] border border-[var(--border-subtle)] text-[var(--text-faint)] px-1.5 py-0.5 rounded text-[10px]">{b.cost_center_code}</span>
                      )}
                      {b.emirate && <span className="text-[var(--text-faint)]">{EMIRATE_LABELS[b.emirate] ?? b.emirate}</span>}
                    </div>
                    <div className="flex items-center gap-4">
                      <span className="text-[var(--text-main)] font-semibold">{fmt(b.revenue)}</span>
                  ...
                  <div className="flex items-center justify-between text-[10px] text-[var(--text-faint)] mt-1">
      ...
      {/* Detailed table */}
      {loading ? (
        <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-2xl p-12 text-center text-[var(--text-faint)] text-sm">Loading…</div>
      ) : branches.length === 0 ? (
        <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-2xl p-16 text-center">
          <p className="text-4xl mb-3">📊</p>
          <p className="text-[var(--text-main)] font-medium">No data for this period</p>
          <p className="text-[var(--text-faint)] text-sm mt-1">Select a tenant with branches and invoiced transactions to see the P&amp;L</p>
        </div>
      ) : (
        <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-2xl overflow-hidden">
          <div className="px-6 py-4 border-b border-[var(--border-subtle)]">
            <h2 className="text-[var(--text-main)] font-semibold">Branch P&amp;L Breakdown</h2>
          </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-white/5">
                {branches.map(b => {
                  ...
                  return (
                    <tr key={b.branch_id ?? 'unassigned'} className="hover:bg-[var(--bg-surface-hover)] transition-colors">
                        ...
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                              {b.emirate && <span className="text-[var(--text-faint)] text-xs">{EMIRATE_LABELS[b.emirate] ?? b.emirate}</span>}
                              {b.cost_center_code && <span className="font-mono text-[var(--text-faint)] text-xs">{b.cost_center_code}</span>}
                              {b.trade_license_no && <span className="text-slate-700 text-xs">{b.trade_license_no}</span>}
                            </div>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-4 text-right text-[var(--text-main)] font-semibold">{fmt(b.revenue)}</td>
                      ...
                      <td className="px-4 py-4 text-right text-[var(--text-muted)]">{fmt(b.vat_collected)}</td>
                      <td className="px-4 py-4 text-right text-rose-400">{b.expenses > 0 ? fmt(b.expenses) : <span className="text-[var(--text-faint)]">—</span>}</td>
                      <td className="px-4 py-4 text-right font-semibold text-[var(--text-main)]">{fmt(b.gross_profit)}</td>
                ...
                {totals && (
                  <tr className="bg-emerald-500/5 border-t border-emerald-500/20 font-semibold">
                    <td className="px-6 py-4 text-[var(--text-main)]">CONSOLIDATED TOTAL</td>
                    ...
                    <td className="px-4 py-4 text-right text-[var(--text-muted)]">{fmt(totals.vat_collected)}</td>
                    <td className="px-4 py-4 text-right text-rose-400">{totals.expenses > 0 ? fmt(totals.expenses) : '—'}</td>
                    <td className="px-4 py-4 text-right text-[var(--text-main)]">{fmt(totals.gross_profit)}</td>
                    <td className="px-4 py-4 text-right text-emerald-400">{totals.margin_pct}%</td>
                    <td className="px-4 py-4 text-right text-[var(--text-muted)]">{totals.invoice_count}</td>
                  </tr>
                )}
              </tbody>
            </table>
