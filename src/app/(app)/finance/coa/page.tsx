    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-2xl w-full max-w-lg">
        <div className="flex items-center justify-between p-5 border-b border-[var(--border-subtle)]">
          <h2 className="text-lg font-bold text-[var(--text-main)]">Add Account</h2>
          <button onClick={onClose} className="text-[var(--text-muted)] hover:text-[var(--text-main)] text-xl">×</button>
        </div>
        <div className="p-5 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-[var(--text-muted)] mb-1">Account Code *</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-[var(--text-muted)] mb-1">Parent Account</label>
            ...
            <div>
              <label className="block text-xs text-[var(--text-muted)] mb-1">Sub-type</label>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-sm text-[var(--text-muted)]">Header / Group account (no direct posting)</span>
          </label>
        </div>
...
function AccountRow({ account, depth = 0, typeFilter }: { account: CoaAccount; depth?: number; typeFilter: string }) {
  ...
  return (
    <>
      <tr className={`border-b border-[var(--border-subtle)] hover:bg-[var(--bg-surface)]/40 transition-colors ${account.is_header ? 'bg-[var(--bg-surface)]/20' : ''}`}>
        <td className="px-4 py-2.5" style={{ paddingLeft: `${16 + depth * 20}px` }}>
          <div className="flex items-center gap-2">
            {hasChildren ? (
              <button onClick={() => setExpanded(!expanded)} className="text-[var(--text-muted)] hover:text-[var(--text-main)] text-xs w-4">
                {expanded ? '▼' : '▶'}
              </button>
            ) : <span className="w-4" />}
            <span className={`font-mono text-xs font-bold ${account.is_header ? 'text-[var(--text-main)]' : 'text-[var(--text-muted)]'}`}>
              {account.account_code}
            </span>
          </div>
        </td>
        <td className="px-4 py-2.5">
          <span className={`text-sm ${account.is_header ? 'font-semibold text-[var(--text-main)]' : 'text-[var(--text-muted)]'}`}>
            {account.account_name}
          </span>
          {account.is_system && <span className="ml-2 text-xs text-[var(--text-faint)]">⚙ sys</span>}
          {account.description && <p className="text-xs text-[var(--text-faint)] mt-0.5 max-w-sm truncate">{account.description}</p>}
        </td>
        <td className="px-4 py-2.5">
        ...
        <td className="px-4 py-2.5 text-xs text-[var(--text-muted)]">{account.account_subtype?.replace('_', ' ') ?? '—'}</td>
        <td className="px-4 py-2.5">
          <span className={`text-xs font-medium ${account.is_header ? 'text-[var(--text-faint)]' : 'text-[var(--text-muted)]'}`}>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[var(--text-main)]">Chart of Accounts</h1>
          <p className="text-[var(--text-muted)] text-xs mt-0.5">Transport-specific double-entry COA — {flat.length} accounts</p>
        </div>
      ...
      <div className="grid grid-cols-5 gap-3">
        {typeCounts.map(tc => (
          <button key={tc.type} onClick={() => setTypeFilter(typeFilter === tc.type ? 'ALL' : tc.type)}
            className={`p-4 rounded-2xl border transition-all text-left ${typeFilter === tc.type ? TYPE_COLORS[tc.type] + ' border-2' : 'bg-[var(--bg-surface)]/60 border-[var(--border-subtle)] hover:border-[var(--border-strong)]'}`}>
            <p className="text-xs text-[var(--text-muted)]">{tc.type}</p>
            <p className="text-2xl font-bold text-[var(--text-main)] mt-1">{tc.count}</p>
            <p className="text-xs text-[var(--text-faint)] mt-0.5">accounts</p>
          </button>
        ))}
      </div>
      ...
      <div className="flex items-center gap-3">
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by code or name…"
          className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-xl px-4 py-2 text-sm text-[var(--text-main)] w-64 focus:outline-none focus:border-purple-500" />
        <div className="flex bg-[var(--bg-surface)]/60 border border-[var(--border-subtle)] rounded-xl p-1 gap-1">
          {TYPE_LABELS.map(t => (
            <button key={t} onClick={() => setTypeFilter(t)}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${typeFilter === t ? 'bg-purple-600 text-white' : 'text-[var(--text-muted)] hover:text-[var(--text-main)]'}`}>
              {t}
            </button>
          ))}
        </div>
      ...
      {/* Accounts Tree Table */}
      {loading ? (
        <div className="h-96 bg-[var(--bg-surface)]/60 rounded-2xl animate-pulse" />
      ) : (
