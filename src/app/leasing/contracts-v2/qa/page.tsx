  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-4 h-[calc(100vh-100px)] flex flex-col">
      ...
      <div className="bg-[var(--bg-surface)]/40 border border-[var(--border-subtle)] rounded-xl p-4">
        <label className="text-xs text-[var(--text-muted)] mb-2 block">Contract</label>
        <select
          value={contractId}
      ...
      {/* Chat panel */}
      <div className="flex-1 bg-[var(--bg-surface)]/40 border border-[var(--border-subtle)] rounded-xl flex flex-col overflow-hidden">
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3">
          ...
          {busy && (
            <div className="flex items-center gap-2 text-[var(--text-faint)] text-sm pl-2">
        ...
        {contractId && messages.length === 0 && (
          <div className="p-4 border-t border-[var(--border-subtle)] bg-[var(--bg-surface)]/30">
            <div className="text-xs text-[var(--text-faint)] mb-2">Try a question:</div>
            <div className="flex flex-wrap gap-2">
              {[...SUGGESTED_QUESTIONS_EN, ...SUGGESTED_QUESTIONS_AR].map((q) => (
                <button
                  ...
                  onClick={() => ask(q)}
                  disabled={busy}
                  className="text-xs px-3 py-1 rounded-full bg-[var(--bg-surface-hover)]/60 hover:bg-[var(--bg-surface-hover)]/60 text-[var(--text-main)] border border-[var(--border-strong)] transition disabled:opacity-50"
                >
                  {q}
                </button>
              ))}
        ...
        >
          <input
            ...
            className="flex-1 px-4 py-2 rounded-lg bg-[var(--bg-surface)]/60 border border-[var(--border-subtle)] text-[var(--text-main)] placeholder-[var(--text-faint)] disabled:opacity-50"
          />
          <button
            type="submit"
  ...
  if (msg.role === 'user') {
    return (
      <div className="flex justify-end">
        <div className="max-w-[80%] rounded-2xl rounded-tr-md bg-blue-600 text-[var(--text-main)] px-4 py-2 text-sm whitespace-pre-wrap">
          {msg.text}
        </div>
      </div>
    );
  ...
  return (
    <div className="flex justify-start">
      <div className="max-w-[85%]">
        <div className="rounded-2xl rounded-tl-md bg-[var(--bg-surface-hover)]/60 text-[var(--text-main)] px-4 py-3 text-sm whitespace-pre-wrap leading-relaxed">
          {msg.text}
        </div>
        {(msg.toolsCalled && msg.toolsCalled.length > 0) || msg.durationMs ? (
          <div className="mt-1 ml-1 flex items-center gap-2 flex-wrap text-[10px] text-[var(--text-faint)]">
