...
// ─── Input helper ─────────────────────────────────────────────────────────────
const inputCls = 'w-full bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-lg px-3 py-2 text-sm text-[var(--text-main)] placeholder-[var(--text-faint)] focus:outline-none focus:border-violet-500';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-[var(--bg-surface)] border border-blue-500/20 rounded-xl p-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
        ...
        <input
          ...
          className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-lg px-3 py-1.5 text-sm text-[var(--text-main)] placeholder-[var(--text-faint)] focus:outline-none focus:border-violet-500 w-64"
        />
      </div>
                    ...
                    <div className="flex flex-col gap-1">
                      {t.status === 'REQUESTED' && (
                        <>
                          <button
                            ...
                            className="px-2.5 py-1 rounded text-xs font-medium bg-blue-600 hover:bg-blue-500 text-[var(--text-main)] transition-colors"
                          >Approve</button>
                          <button
                      ...
                      {t.status === 'APPROVED' && (
                        <button
                          ...
                          className="px-2.5 py-1 rounded text-xs font-medium bg-indigo-600 hover:bg-indigo-500 text-[var(--text-main)] transition-colors whitespace-nowrap"
          ...
          <div className="bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-2xl w-full max-w-sm shadow-2xl">
            ...
            <div className="flex gap-3 p-5 border-t border-[var(--border-subtle)]">
              ...
              <button onClick={() => handleAction(showApprove.id, 'APPROVE', { approvedBy })} disabled={saving || !approvedBy.trim()}
                className="flex-1 px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-500 text-[var(--text-main)] text-sm font-medium transition-colors disabled:opacity-50">
