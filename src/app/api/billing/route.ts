...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const [mrrRow] = await prisma.$queryRawUnsafe<MrrRow[]>(
      `SELECT
    ...
    const byModule = await prisma.$queryRawUnsafe<ModuleRow[]>(
      `SELECT
         module_code,
         COUNT(*) FILTER (WHERE status = 'ACTIVE')                          AS active_count,
    ...
    const recentRuns = await prisma.$queryRawUnsafe<RunRow[]>(
    ...
    const upcomingRenewals = await prisma.$queryRawUnsafe<RenewalRow[]>(
      `SELECT
    ...
    const outstandingInvoices = await prisma.$queryRawUnsafe<OutstandingRow[]>(
      `SELECT
    ...
    const statusBreakdown = await prisma.$queryRawUnsafe<StatusRow[]>(
