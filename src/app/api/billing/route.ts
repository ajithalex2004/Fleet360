...
// ---------------------------------------------------------------------------
// GET /api/billing
// ?type=dashboard            → platform-wide billing dashboard
  ...
  return withTenantRls(prisma, tenantId, async (tx) => {
    const { searchParams } = new URL(req.url);
      ...
      if (type === 'tenant_billing') {
        ...
        const invoices = await tx.$queryRawUnsafe<Row[]>(
          `SELECT
        ...
        type SubRow = {
          module_code: string; status: string; plan_tier: string;
          billing_cycle: string; base_price: string | number;
          currency: string; next_billing_date: Date | string;
          last_billed_date: Date | string | null;
        };
        const subscriptions = await tx.$queryRawUnsafe<SubRow[]>(
      ...
      try {
        ...
        const [mrrRow] = await tx.$queryRawUnsafe<MrrRow[]>(
          `SELECT
             COUNT(*) FILTER (WHERE status = 'ACTIVE')                        AS active_count,
             COALESCE(SUM(base_price) FILTER (
               WHERE status = 'ACTIVE' AND billing_cycle = 'MONTHLY'), 0)     AS monthly_total,
             COALESCE(SUM(base_price / 12.0) FILTER (
               WHERE status = 'ACTIVE' AND billing_cycle = 'ANNUAL'), 0)      AS annual_as_monthly
           FROM tenant_module_subscriptions`
        ).catch(() => [{ active_count: BigInt(0), monthly_total: 0, annual_as_monthly: 0 }]);

        const activeCount = Number(mrrRow?.active_count ?? 0);
        const mrr = Math.round(
          (Number(mrrRow?.monthly_total ?? 0) + Number(mrrRow?.annual_as_monthly ?? 0)) * 100
        ) / 100;
        const arr = Math.round(mrr * 12 * 100) / 100;

        // 2. Subscriptions by module breakdown
        type ModuleRow = {
          module_code: string;
          active_count: bigint;
          trial_count:  bigint;
        ...
        const byModule = await tx.$queryRawUnsafe<ModuleRow[]>(
          `SELECT
             module_code,
             COUNT(*) FILTER (WHERE status = 'ACTIVE')                          AS active_count,
             ...
             + COALESCE(SUM(base_price / 12.0) FILTER (
               WHERE status = 'ACTIVE' AND billing_cycle = 'ANNUAL'), 0)        AS mrr_contribution
           FROM tenant_module_subscriptions
           GROUP BY module_code
           ORDER BY module_code`
        ).catch(() => [] as ModuleRow[]);

        // 3. Recent billing runs (last 5)
        type RunRow = {
          id: string; run_date: Date; status: string;
          total_tenants: number | bigint; invoices_created: number | bigint;
          total_amount: string | number; completed_at: Date | null;
        };
        const recentRuns = await tx.$queryRawUnsafe<RunRow[]>(
        ...
        type RenewalRow = {
          id: string; tenant_id: string; module_code: string;
          plan_tier: string; billing_cycle: string; base_price: string | number;
          currency: string; next_billing_date: Date | string;
          tenant_name: string; tenant_code: string | null;
        };
        const upcomingRenewals = await tx.$queryRawUnsafe<RenewalRow[]>(
          `SELECT
        ...
        // 5. Outstanding subscription invoices (SUB- prefix, not PAID)
        type OutstandingRow = {
          id: string; invoice_number: string; client_name: string;
          total_amount: string | number; payment_status: string;
          due_date: Date | string | null; issue_date: Date | string;
          module_source: string | null; tenant_id: string | null;
        };
        const outstandingInvoices = await tx.$queryRawUnsafe<OutstandingRow[]>(
          `SELECT
        ...
        // 6. Status breakdown across all subscriptions
        type StatusRow = { status: string; cnt: bigint };
        const statusBreakdown = await tx.$queryRawUnsafe<StatusRow[]>(
          `SELECT status, COUNT(*) AS cnt FROM tenant_module_subscriptions GROUP BY status`
        ).catch(() => [] as StatusRow[]);

        const statusCounts: Record<string, number> = {};
        for (const s of statusBreakdown) statusCounts[s.status] = Number(s.cnt);

        // 7. Outstanding totals
        ...
        return NextResponse.json({
          ...
          by_module: byModule.map(m => ({
            ...
            mrr_contribution: Math.round(Number(m.mrr_contribution) * 100) / 100,
          })),
...
export async function POST(req: NextRequest) {
  ...
  // Forward hint for misrouted billing run requests
