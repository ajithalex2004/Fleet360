...
export async function GET(
  ...
  return withTenantRls(prisma, tenantId, async (tx) => {
    const { id: invoiceId, paymentId } = await params;
      ...
      try {
        const payment = await tx.rentalInvoicePayment.findUnique({
          where: { id: paymentId },
          include: { invoice: true },
        });
        if (!payment || payment.invoiceId !== invoiceId) return jsonErr('Receipt not found', 404);

        const customer = await tx.rentalCustomer.findUnique({ where: { id: payment.invoice.customerId } });
        const agreement = await tx.rentalAgreement.findUnique({ where: { id: payment.invoice.agreementId } });
