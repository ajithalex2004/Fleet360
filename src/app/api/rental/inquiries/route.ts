  ...
  return withTenantRls(prisma, tenantId, async (tx) => {
    await ensureSchema();
      ...
      const [rows, counts] = await Promise.all([
        tx.$queryRawUnsafe<Row[]>(`SELECT * FROM rental_inquiries ${where} ORDER BY created_at DESC LIMIT $${pi} OFFSET $${pi+1}`, ...params, limit, offset).catch(()=>[]),
        tx.$queryRawUnsafe<{status:string;count:string}[]>(`SELECT status, COUNT(*)::text as count FROM rental_inquiries WHERE deleted_at IS NULL GROUP BY status`).catch(()=>[]),
      ]);
...
export async function POST(req: NextRequest) {
  ...
  return withTenantRls(prisma, tenantId, async (tx) => {
    await ensureSchema();
      ...
      try {
        [row] = await tx.$queryRawUnsafe<Row[]>(
...
export async function PATCH(req: NextRequest) {
  ...
  return withTenantRls(prisma, tenantId, async (tx) => {
    await ensureSchema();
      ...
      params.push(id);
      const [row] = await tx.$queryRawUnsafe<Row[]>(`UPDATE rental_inquiries SET ${sets.join(',')} WHERE id=$${pi} RETURNING *`, ...params).catch(()=>[]);
