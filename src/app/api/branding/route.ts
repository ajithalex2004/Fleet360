...
import { getBranding, getBrandingByCode, getBrandingByDomain } from '@/lib/branding';
...
export async function GET(req: NextRequest) {
  ...
  let branding = null;
  if (code) {
    branding = await getBrandingByCode(code.trim());
  } else if (domain) {
    branding = await getBrandingByDomain(domain.trim());
  } else {
    const authz = requireAuthorizedTenant({ headers: req.headers, nextUrl: req.nextUrl });
    if (!authz.ok) {
      return NextResponse.json({ error: authz.error }, { status: authz.status });
    }
    branding = await getBranding(authz.tenantId);
  }
