...
export async function GET(req: NextRequest) {
  try {
    ...
    const controlTower = await getShipmentControlTower({
      tenantId,
      limit: Number(req.nextUrl.searchParams.get('limit') ?? 100),
    });
    return NextResponse.json(controlTower);
  } catch (error) {
    console.error('[logistics/control-tower GET]', error);
    return NextResponse.json({ error: 'Failed to fetch shipment control tower' }, { status: 500 });
  }
}
