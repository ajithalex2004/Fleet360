...
export async function GET(req: NextRequest) {
  ...
  return withTenantRls(prisma, tenantId, async (tx) => {
      ...
      try {
        ...
        const daily = await tx.$queryRawUnsafe<Array<{ day: string; trips: bigint; passengers: bigint }>>(
          `SELECT
        ...
        // Trips by shift type
        const byShift = await tx.$queryRawUnsafe<Array<{ shift_type: string | null; count: bigint }>>(
        ...
        // Trips by route
        const byRoute = await tx.$queryRawUnsafe<Array<{ name: string; count: bigint; passengers: bigint }>>(
        ...
        const occupancy = await tx.$queryRawUnsafe<Array<{ avg_rate: number }>>(
        ...
        const byHour = await tx.$queryRawUnsafe<Array<{ hour: number; count: bigint }>>(
        ...
        const slaTotals = await tx.$queryRawUnsafe<Array<{
        ...
        const slaByRoute = await tx.$queryRawUnsafe<Array<{
        ...
        const costAggregate = await tx.$queryRawUnsafe<Array<{
        ...
        const methodMix = await tx.$queryRawUnsafe<Array<{ method: string; count: bigint }>>(
