  ...
  const response = await withTenantRls(prisma, tenantId, async (tx) => {
    const cronSecret = process.env.CRON_SECRET;
      ...
      try {
        ...
        const tripAgg = await tx.$queryRawUnsafe<Array<{
        ...
        const incidentAgg = await tx.$queryRawUnsafe<Array<{ driver_id: string; incidents: bigint }>>(
        ...
        for (const s of scores) {
          try {
            ...
            if (existing) {
              await tx.driverPerformance.update({ where: { id: existing.id }, data });
            } else {
              await tx.driverPerformance.create({ data });
            }
            upserted += 1;
          } catch (err) {
            errors += 1;
