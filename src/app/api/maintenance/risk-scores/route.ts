...
export async function GET() {
    ...
    const recentMRs = await prisma.maintenanceRequest.findMany({
        where: {
    ...
    const recentMRs180 = await prisma.maintenanceRequest.findMany({
        where: {
            vehicleId:  { in: vehicleIds },
            deletedAt:  null,
    ...
    const openMRs = await prisma.maintenanceRequest.findMany({
        where: {
            vehicleId: { in: vehicleIds },
            deletedAt: null,
    ...
    const downtimeMRs = await prisma.maintenanceRequest.findMany({
        where: {
            vehicleId:  { in: vehicleIds },
            deletedAt:  null,
    ...
    const activeWarranties = await prisma.vehicleWarranty.findMany({
        where: {
            vehicleId:  { in: vehicleIds },
            isActive:   true,
            startDate:  { lte: now },
