...
export async function POST(request: NextRequest) {
  return tenantBootstrapHandler(request, 'verify_tenant_domain', async ({ tx }) => {
    try {
        ...
        const rows = await tx.$queryRawUnsafe<TenantVerify[]>(
...
export async function GET(request: NextRequest) {
  return tenantBootstrapHandler(request, 'verify_tenant_domain', async ({ tx }) => {
    try {
        ...
        const rows = await tx.$queryRawUnsafe<TenantDns[]>(
        ...
        try {
          ...
          // ENOTFOUND / ENODATA means DNS lookup failed — not an e we throw on
