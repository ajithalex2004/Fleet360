...
export async function GET(req: NextRequest) {
  ...
  return withTenantRls(prisma, tenantId, async (tx) => {
    try {
        const vehicles = await tx.$queryRawUnsafe<Array<{
          id: string;
        ...
        console.error('[vehicles/logistics]', e);
        // Graceful fallback — try generic query without service join
        try {
          const fallback = await tx.$queryRawUnsafe<Array<Record<string, unknown>>>(
            `SELECT id,
          ...
          console.error('[vehicles/logistics fallback]', e);
          return NextResponse.json([]);
        }
      }
  });
