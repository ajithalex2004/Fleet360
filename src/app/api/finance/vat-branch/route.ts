...
export async function GET(req: NextRequest) {
  ...
  return withTenantRls(prisma, tenantId, async (tx) => {
    const { searchParams } = new URL(req.url);
      ...
      const vatByBranch = await tx.$queryRawUnsafe<VatRow[]>(
        `SELECT
           b.id                     AS branch_id,
           b.branch_name,
           b.emirate,
      ...
      const [unassigned] = await tx.$queryRawUnsafe<UnassRow[]>(
        `SELECT
      ...
      const inputVatByBranch = await tx.$queryRawUnsafe<InputRow[]>(
        `SELECT
           b.id          AS branch_id,
           b.branch_name,
