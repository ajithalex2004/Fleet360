...
export async function GET(req: NextRequest) {
  ...
  return withTenantRls(prisma, tenantId, async (tx) => {
    const sp  = req.nextUrl.searchParams;
      ...
      const [glCheck] = await tx.$queryRawUnsafe<{ count: string }[]>(
      ...
      async function glBalance(like: string, normalBalance: 'DEBIT' | 'CREDIT'): Promise<BSLine[]> {
        const rows = await tx.$queryRawUnsafe<{ account_code: string; account_name: string; account_subtype: string | null; balance: string }[]>(
      ...
      async function getAssets(): Promise<{ current: BSLine[]; fixed: BSLine[]; other: BSLine[] }> {
        ...
        // Module fallback
        const [cash]    = await tx.$queryRawUnsafe<{t:string}[]>(`SELECT COALESCE(SUM(amount),0)::text as t FROM finance_payments WHERE deleted_at IS NULL AND status='RECONCILED' AND payment_date <= $1`, asOf).catch(()=>[{t:'0'}]);
        const [ar]      = await tx.$queryRawUnsafe<{t:string}[]>(`SELECT COALESCE(SUM(total_amount - paid_amount),0)::text as t FROM finance_invoices WHERE deleted_at IS NULL AND issue_date <= $1 AND payment_status NOT IN ('PAID','CANCELLED','DRAFT')`, asOf).catch(()=>[{t:'0'}]);
        const [pdc]     = await tx.$queryRawUnsafe<{t:string}[]>(`SELECT COALESCE(SUM(amount),0)::text as t FROM finance_pdc_cheques WHERE deleted_at IS NULL AND direction='INWARD' AND status IN ('HELD','DEPOSITED') AND cheque_date <= $1`, asOf).catch(()=>[{t:'0'}]);
        const [nbv]     = await tx.$queryRawUnsafe<{t:string}[]>(`SELECT COALESCE(SUM(net_book_value),0)::text as t FROM finance_fixed_assets WHERE deleted_at IS NULL AND acquisition_date <= $1 AND status != 'DISPOSED'`, asOf).catch(()=>[{t:'0'}]);
        const [accDep]  = await tx.$queryRawUnsafe<{t:string}[]>(`SELECT COALESCE(SUM(accumulated_depreciation),0)::text as t FROM finance_fixed_assets WHERE deleted_at IS NULL AND acquisition_date <= $1 AND status != 'DISPOSED'`, asOf).catch(()=>[{t:'0'}]);
        return {
          current: [
      ...
      async function getLiabilities(): Promise<{ current: BSLine[]; nonCurrent: BSLine[] }> {
        ...
        const [vat]     = await tx.$queryRawUnsafe<{t:string}[]>(`SELECT COALESCE(SUM(vat_amount),0)::text as t FROM finance_invoices WHERE deleted_at IS NULL AND issue_date <= $1 AND payment_status NOT IN ('PAID','CANCELLED','DRAFT')`, asOf).catch(()=>[{t:'0'}]);
        const [expPay]  = await tx.$queryRawUnsafe<{t:string}[]>(`SELECT COALESCE(SUM(total_amount),0)::text as t FROM finance_expenses WHERE deleted_at IS NULL AND status='APPROVED' AND expense_date <= $1`, asOf).catch(()=>[{t:'0'}]);
        const [pdcOut]  = await tx.$queryRawUnsafe<{t:string}[]>(`SELECT COALESCE(SUM(amount),0)::text as t FROM finance_pdc_cheques WHERE deleted_at IS NULL AND direction='OUTWARD' AND status IN ('HELD','DEPOSITED') AND cheque_date <= $1`, asOf).catch(()=>[{t:'0'}]);
        return {
          current: [
      ...
      async function getEquity(): Promise<BSLine[]> {
        if (hasGl) {
          ...
          const [pl] = await tx.$queryRawUnsafe<{net:string}[]>(
          ...
          const retainedEarnings = toN((Array.isArray(pl) ? pl[0]?.net : pl?.net) ?? '0');
          return [
            ...rows,
        ...
        const [rev]  = await tx.$queryRawUnsafe<{t:string}[]>(`SELECT COALESCE(SUM(subtotal_amount),0)::text as t FROM finance_invoices WHERE deleted_at IS NULL AND issue_date <= $1 AND payment_status NOT IN ('DRAFT','CANCELLED')`, asOf).catch(()=>[{t:'0'}]);
        const [exp]  = await tx.$queryRawUnsafe<{t:string}[]>(`SELECT COALESCE(SUM(total_amount),0)::text as t FROM finance_expenses WHERE deleted_at IS NULL AND status IN ('APPROVED','PAID') AND expense_date <= $1`, asOf).catch(()=>[{t:'0'}]);
        return [
