...
export async function GET(req: NextRequest) {
  ...
  // lease_invoices silently returned zero rows regardless of whether the
  // lessee actually had invoices. Same bug found across every other
  // leasing-portal route that used a bare `prisma.X` call instead of
  // withTenantRls.
  const [invoices, intents] = await Promise.all([
    withTenantRls(prisma, ctx.tenantId, (tx) =>
