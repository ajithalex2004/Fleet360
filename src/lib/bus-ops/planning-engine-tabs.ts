 ...
 *
 * Visual order follows the data flow — operational rules and planning
 * constraints both feed the plan, and Planning Core pre-fills its WorkRules
 * from the default CBA rule-set. But Planning Core is the daily-driver task
 * while the other three are configured rarely, so opening on a config screen
 * would tax the frequent job on every visit.
 */
