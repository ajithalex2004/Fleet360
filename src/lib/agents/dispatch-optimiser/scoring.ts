/**
 * Smart Dispatch Optimiser — 15-Factor Semi-Autonomous Scoring Model v2.0.0
 * --------------------------------------------------------------------------
 ...
 * live HOS shift verification, and maintenance health index integration.
 *
 * Factors and weights:
...
export function scoreCandidate(candidate: DriverCandidate, job: JobRequirements): ScoredCandidate {
...
function buildReason(s: ScoredCandidate, job: JobRequirements): string {
  ...
  parts.push(`${s.vehicleCode} is ${s.distanceKm} km away (ETA ${s.etaMinutes} mins)`);
