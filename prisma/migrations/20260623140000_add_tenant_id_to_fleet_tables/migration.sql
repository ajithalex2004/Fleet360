...
--
-- The backfill picks the oldest active tenant. Reasoning: on a system
-- with a single tenant (most installs at this point), this assigns
-- everything to that tenant — correct by default. On a multi-tenant
-- system the operator should provision data ownership BEFORE running
-- this migration; the loud NOT NULL failure in step 3 ensures they don't
-- proceed silently with wrong assignments.

-- Resolve the default tenant once, error loudly if none exists AND there's
-- data to backfill. Cached in a temp setting so each table uses the same
-- value (consistency across joins).
DO $$
DECLARE
...
BEGIN
  ...
  IF default_tenant_id IS NULL AND has_any_data THEN
    RAISE EXCEPTION
  ...
  PERFORM set_config('fleet360.default_tenant_id', COALESCE(default_tenant_id, ''), false);
END $$;
...
UPDATE "vehicles" SET "tenant_id" = NULLIF(current_setting('fleet360.default_tenant_id'), '') WHERE "tenant_id" IS NULL;
...
UPDATE "drivers" SET "tenant_id" = NULLIF(current_setting('fleet360.default_tenant_id'), '') WHERE "tenant_id" IS NULL;
...
UPDATE "garages" SET "tenant_id" = NULLIF(current_setting('fleet360.default_tenant_id'), '') WHERE "tenant_id" IS NULL;
...
UPDATE "maintenance_requests" SET "tenant_id" = NULLIF(current_setting('fleet360.default_tenant_id'), '') WHERE "tenant_id" IS NULL;
...
UPDATE "histories" SET "tenant_id" = NULLIF(current_setting('fleet360.default_tenant_id'), '') WHERE "tenant_id" IS NULL;
...
UPDATE "service_requests" SET "tenant_id" = NULLIF(current_setting('fleet360.default_tenant_id'), '') WHERE "tenant_id" IS NULL;
...
UPDATE "quotations" SET "tenant_id" = NULLIF(current_setting('fleet360.default_tenant_id'), '') WHERE "tenant_id" IS NULL;
...
UPDATE "alerts" SET "tenant_id" = NULLIF(current_setting('fleet360.default_tenant_id'), '') WHERE "tenant_id" IS NULL;
...
UPDATE "alert_configs" SET "tenant_id" = NULLIF(current_setting('fleet360.default_tenant_id'), '') WHERE "tenant_id" IS NULL;
...
UPDATE "attachments" SET "tenant_id" = NULLIF(current_setting('fleet360.default_tenant_id'), '') WHERE "tenant_id" IS NULL;
...
UPDATE "comments" SET "tenant_id" = NULLIF(current_setting('fleet360.default_tenant_id'), '') WHERE "tenant_id" IS NULL;
